...
<template>
    <VDropdown theme="detail" alt="Quick Search" :distance="8"
        class="no-drag search-drop ghosty drag-label flex basis-16 !p-0 group/qs after:grid after:place-content-center after:w-full after:h-full after:content-['+'] after:absolute relative hover:after:text-neutral z-0 hover:after:opacity-60 after:opacity-50 !cursor-pointer"
            ...
            <div class="relative pt-1.5 pb-0">
                <div class="relative flex justify-center w-full px-2 pb-1">
                    ...
                    <input placeholder="Quick search..." :value="quickSearch" @input="handleInput" spellcheck="false"
                        class="z-0 w-full text-xs italic select-all focus:not-italic focus:bg-base-200/50 placeholder:text-xs rounded-btn bg-transparent h-7 px-2 !outline-none"
                        autofocus />
                </div>
