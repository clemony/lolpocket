...
<template>
  <div class="fixed right-22 bottom-22 flex flex-col gap-6">
    <Tooltip
      ...
      :key="ability.name" :text="ability.name" side="left"
    >
      <BtnLink
        color="neutral"
        :class="
          cn(
            'size-20! rounded-full overflow-hidden border-0 p-0 shadow-sm shadow-black/10 drop-shadow-sm drop-shadow-black/30 *:pointer-events-none',
            {
