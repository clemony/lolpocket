...
<template>
<div v-if="champion" class="mt-2 grid !max-h-full grid-cols-1 grid-rows-[min-content_1fr_min-content] gap-4">
    <div
        class="shadow-inset-sm grid h-28 w-full grid-cols-2 rounded-box border border-base-300/70 bg-base-100/80 bg-[50%] px-1 py-3">
        <div class="col-span-2 grid grid-cols-3 gap-1 p-1">
            <div
                class="align-self-start justify-content-center avatar relative col-start-1 -mt-1.5 ml-1.5 !aspect-square h-16">
                <div class="justify-content-center z-0 my-auto grid h-16 w-16 rounded-full object-contain">
                    <svg role="none" class="z-10 h-full w-full">
                        <mask id="circle">
                        ...
                        <g mask="url(#circle)">
                            <image x="-6px" y="-6px" height="115%" width="115%" class="max-w-[115%]"
                                :href="`/img/champions/${champion.name.replace(/\s/g, '').replace(/\'/g, '').replace(/\./g, '').replace(/\&/g, '')}.webp`">
                            </image>
                    ...
                    <div theme="minitt"
                        class="group/glow glow absolute -bottom-[4px] -right-[5px] grid size-7 items-center justify-center rounded-full transition-all duration-500 before: hover:rotate-180"
                        ...
                        <a :href="champion.wiki" target="_blank"
                            class="badge relative z-10 mb-[2px] flex aspect-square size-[22px] cursor-alias overflow-hidden rounded-full border-none bg-base-content object-cover p-[3px] text-base-100 ring-[3px] ring-base-100 hover:ring-transparent">
                            <Icon icon="ph:link-simple" class="" />
                        </a>
                    </div>
                </div>
            </div>
            <div class="relative col-span-2 col-start-2 -ml-1 -mt-1 antialiased">
                <h1 class="mb-0.5 pb-0 !text-xl font-semibold tracking-tight antialiased">
                    {{ champion.name }}
                </h1>
        ...
        <div class="justify-content-end relative right-3 col-start-2 flex gap-2 justify-self-end pb-1 text-center ">
            <code v-for="(tag, index) in champion.tags" :key="index"
                class="badge flex h-max w-auto flex-nowrap text-center font-mono  lowercase tracking-normal"
    ...
    <TabList :modelName="cTabs" :defaultTab="2">
        <!----------------------⟢ stats ⟣---------------------------->
