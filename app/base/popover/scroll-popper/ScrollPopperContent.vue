...
<template>
  <PopoverPortal>
    <PopoverContent
      ...
      <PopperScrollUpButton :class="{ 'opacity-0 pointer-events-none  transition-opacity duration-300': arrivedState.top == true }" />

      <div
        ref="el"
        :class="cn('max-h-66  min-w-[--reka-popover-trigger-width] overflow-auto size-full', props.class)">
        <div
          :class="cn('p-2 min-w-[--reka-popover-trigger-width] size-full')">
          <slot />
        </div>
      </div>

      <PopperScrollDownButton :class="{ 'opacity-0 pointer-events-none  transition-opacity duration-300': arrivedState.bottom == true }" />
    </PopoverContent>
  </PopoverPortal>
</template>