...
export async function useSearch(
  ...
  const pages = computed(() => {
    const results = computed(() =>
      !resultsLength || resultsLength === undefined
        ? pageResults.value
        : router
            .getRoutes()
            .filter(
              (r) =>
                r.meta?.search !== "hidden" && r.path.split("/").length === 2,
            ),
    );
    ...
    results.value
      ...
      .forEach((parent) => {
        g.value.push({
          ...
          items: (parent.children as RouteRecordRaw[]).sort(
