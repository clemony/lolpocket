...
<template>
  ...
  >
    <template v-if="!comment.removed">
      <template v-if="comment && comment.author_id === as().account.puuid">
        <Toggle
        ...
        <span class="relative grid size-2 place-items-center opacity-40">
          <icon name="slash" class="absolute size-4" />
        </span>
        ...
        <Button
          ...
          size="max"
          class="inline px-1 align-bottom text-1 opacity-30 disabled:hidden"
