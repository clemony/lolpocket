...
<template>
    ...
    <ColorPicker :color="color" class="w-full col-start-1" @color-change="updateColor" alpha-channel="hide">
    ...
    <div class="flex items-center gap-4 mx-3 mb-2 justify-self-end">
        ...
        <div class=" rounded-full justify-self-end self-end join *:text-xs">
            ...
            <label class="join-item btn btn-xs  has-[:checked]:btn-neutral">
                <input type="radio" name="colorType" value="background" class="hidden peer" v-model="colorType" />
                Background
            </label>
            <label class="join-item btn btn-xs has-[:checked]:btn-neutral">
                <input type="radio" name="colorType" v-model="colorType" value="icon" class="hidden peer" />
                Icon
            </label>
        </div>
