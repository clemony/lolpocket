    ...
    <LazyDialogContent class="fixed top-1/3! mt-0! translate-y-0! p-0">
      ...
      <Command :highlight-on-hover="true">
        <Input
          ...
          type="text"
          placeholder="Search..."
          class="peer relative flex field-sizing-content h-14 w-auto min-w-36 grow rounded-b-none! border-b-b3 ring-0! outline-0!"
        ...
        <CommandList>
          <TransitionExpand group>
            ...
            <!-- champions -->
            <CommandGroup v-if="champions.length" heading="Champions">
              <CommandItem
                v-for="option in champions"
                :key="option.name"
                class="w-full gap-3 pl-3 capitalize"
                :value="option"
            ...
            <!-- pockets -->
            <CommandGroup v-if="pockets.length" heading="Pockets">
              <CommandItem
