...
<template>
  <UCard
    as-child
    :ui="{
      root: 'w-full rounded-4xl border-p2 shadow-none inset-ring-0 drop-shadow-none',
    }">
    <UCollapsible
      :default-open="true"
      :ui="{
      ...
      <UButton
        ...
        block
        :ui="{
          base: 'justify-between border-0 open:rounded-b-none open:border-b open:border-b-p2 open:bg-transparent! open:fx-0!',
          label: 'grow-0 text-pc/50 group-hover/btn:text-pc',
          trailingIcon:
      ...
      <template #content>
        <Listbox v-model:model-value="model" :multiple="false">
          <ListboxContent
            :class="
              cn(
                'i9 z-auto h-max max-h-100 w-full space-y-1 overflow-y-auto rounded-xl px-1.5 py-3'
              )
            ">
            <template v-if="!sMatches().loading && sMatches.length">
              <ListboxItem
                ...
                as-child
                :class="
                  cn('', { 'sticky top-0 z-2': item.championId === model })
                "
