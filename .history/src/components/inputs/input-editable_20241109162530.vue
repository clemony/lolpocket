...
<EditableRoot v-slot="{ isEditing }" :default-value='props.defaultValue'
    ...
    <EditableArea class=" w-[190px] relative">
        <EditablePreview />
        <EditableInput class="w-full peer cursor-text" v-model="modelValue" spellcheck='false' />
        <div class="absolute right-2 h-full opacity-0 peer-focus:opacity-100 transition-all duration-300">
            <slot />
        </div>
    </EditableArea>
</EditableRoot>
