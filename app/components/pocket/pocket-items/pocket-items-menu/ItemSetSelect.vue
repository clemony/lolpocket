...
<template>
  <Field :title="props.title">
    <Select v-model:model-value="pocket.complete.items[props.index]">
      <SelectTrigger class="h-auto shadow-none py-4 hover:border-b2 !bg-b2/40 hover:inset-shadow-xs hover:shadow-warm-2 cursor-pointer !rounded-box" :disabled="pocket.items.sets.length == 0">
      ...
      <SelectContent position="popper" class="w-[var(--reka-select-trigger-width)] !rounded-box">
        <SelectGroup>
          <SelectItem v-for="set in pocket.items.sets" :key="set.name" :value="set" class="rounded-xl">
