...
<EditableRoot v-slot="{ isEditing }" :default-value='props.defaultValue'
    ...
    placeholder="Enter text..." class=" gap-4" auto-resize :style="{
        width: inputWidth + 'px'
    }">
    <slot name="outer" />
    <EditableArea class=" w-fit relative">
        <EditablePreview />
        <EditableInput class="w-fit peer " v-model="modelValue" spellcheck='false' @input="updateWidth" />
