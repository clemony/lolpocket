  ...
  <Collapsible v-if="pocket" class="w-full field-box">
    <CollapsibleTrigger
      ...
      <VueDraggable
        ...
        <RoleButton v-for="role in pocket.roles" :key="role" :pocket="pocket" :role="role" />
    ...
    <CollapsibleContent class="CollapsibleContent flex flex-col gap-4 justify-start overflow-hidden  w-full px-4  pt-3 pb-6">
      <transition-slide :offset="{ enter: [10, 0], leave: [-10, 0] }" group class="flex flex-wrap gap-4 relative w-full h-fit">
        ...
        <template
          ...
          <label
            v-if="role"
            class="border-b3/80 btn btn-sm order-3 font-medium   mr-0 " :class="cn({ 'hidden ': pocket.roles.includes(role) })">
            ...
            <component
              ...
              class="h-3.75 text-bc/70 w-auto dst shrink-0 peer-checked:text-nc" :class="{ '!text-bc/80': role.toLowerCase() == 'all' }" />
            {{ role }}
          </label>
        </template>
      </transition-slide>
