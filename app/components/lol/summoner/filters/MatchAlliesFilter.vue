      ...
      <ListboxContent class="w-full py-4">
        <template v-if="!loading">
          ...
          >
            <Button
              size="12"
              variant="ghost"
              class="grid w-full grid-cols-[4fr_repeat(2,1fr)] gap-3 px-5 dst duration-0! **:font-medium hover:shadow-xs"
            >
              <div
                class="flex grow items-center gap-3 justify-self-start text-start text-2"
              >
                <SummonerIcon
        ...
        <template v-else>
          <div
            v-for="i in 10"
            :key="i"
            class="btn pointer-events-none grid w-full grid-cols-[20px_1fr] items-center justify-items-end gap-3 py-3 opacity-60 btn-ghost"
          >
