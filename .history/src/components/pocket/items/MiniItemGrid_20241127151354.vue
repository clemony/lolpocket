...
<template>
    <template v-for="(item, index) in items" :key="item.id">
        <div
            v-if="index < 6"
            :key="item.id"
            class="border-b3/70 bg-b1/50 shadow-warm shrink-0 rounded-lg border p-0 backdrop-blur-sm [&_first-child]:rounded-tl-full!">
            <Item
                :item="item"
                :pocket="pocket"
                imgClass="rounded-lg shadow-xs drop-shadow-sm p-0! m-0 object-center  size-full border border-b3"
