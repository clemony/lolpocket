...
<template><label role="tab" :class="{ 'tab-active': model === props.defaultTab }"
    class="tab !z-20 text-base absolute max-w-24 first:left-24  top-0 left-0  [--tab-border-color:oklch(var(--b2))]">
    <slot name="header" />
</label>

<div v-if="tabValue === props.modelName" role="tabpanel"
    :class="{ 'rounded-tl-none !max-w-full': props.modelName === props.first }"
