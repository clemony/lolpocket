    ...
    <VDropdown
        ...
        <template #popper>
            <template v-for="column in ps.columns" :key="column.colId">
                <VDropdown
                    v-if="
                        column.headerName == 'Updated' ||
                        column.headerName == 'Created'
                    "
                    theme="hover-inner">
                    <Label
                        class="hover:bg-b2/60 flex w-full items-center gap-4 rounded-md px-3 py-1 text-sm">
                        <input
                            type="checkbox"
                            :checked="column.isVisible"
                            @change="
                                toggleColumnVisibility(column.colId, $event)
                            "
