...
<template>
  <menu
    class="rounded-t-3xl rounded-b-box  w-13 h-13 bg-neutral/0 z-41 fixed  top-2 right-3  group justify-self-end backdrop-blur-md" :class="{ 'ease-in-out  hover:h-54  hover:bg-neutral/85 transition-[height_colors] dr-40 ': as.currentSession.session }">
    ...
    <div class="opacity-0 w-13 group-hover:opacity-100 transition-opacity dr-30 absolute h-full ml-2.5 items-center justify-end flex flex-col *:size-11 pt-5 pb-1  z-0 overflow-hidden">
      <button v-tippy="{ content: 'New Pocket', placement: 'bottom' }" class="   h-full group/b cursor-pointer size-full ml-0.25  -mb-1.5 place-items-center flex " @click="handleAddClick()">
        <icon
      ...
      <button v-tippy="{ content: 'Settings', placement: 'bottom' }" class="   h-full group/b cursor-pointer">
        <NuxtLink to="/settings" class="size-full ml-px   place-items-center flex ">
          <icon
      ...
      <button v-tippy="{ content: 'Sign Out', placement: 'bottom' }" class="flex   h-full  place-items-center ml-px group/b cursor-pointer" @click="handleClick('out')">
        <icon
