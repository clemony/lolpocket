...
import { constructMentionAttrs } from '../constructMentionAttrs'
...
const item = computed(() => node.attrs)
</script>
...
<template>
  <NodeViewWrapper class="inline w-fit">
    ...
    <div
      v-if="item"
      v-bind="constructMentionAttrs(item)"
      size="6">
      <IndexIcon :item="attrsToProps(item)" />
      {{ item['data-name'] }}
    </div>
