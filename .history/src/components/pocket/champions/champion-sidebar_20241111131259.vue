...
<div v-if="champion" class="flex-col w-full gap-6 max-w-inherit h-full pr-[3px]">
    ...
    <Card class='relative'>
        <div class='join-item join join-horizontal overflow-hidden h-9  absolute top-3 right-2 w-20'>

            <a :href="champion.wiki" target="_blank" class='w-1/2  join-item rounded-t-none'>
                <Button variant="outline" class="items-center justify-center join-item size-full p-0  group">
                    <Icon icon="ph:link-simple" class=" !size-9 duration-500 transition-all  group-hover:rotate-180" />
                </Button>
            </a>
