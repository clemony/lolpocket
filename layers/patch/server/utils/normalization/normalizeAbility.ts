...
function cleanModifiers(
  modifiers: DirtyModifier[] | null | undefined
): CleanedModifier[] {
  ...
  const normalized = modifiers
    .map((m) => {
      const values: number[] =
        m.values?.filter((v): v is number => v !== null && v !== undefined) ??
        []
      const units = m.units ?? []
      ...
      const rounded = cleanedValues.map((v) =>
...
function simplifyArray<T>(arr: T[]): T[] {
  return arr.length && arr.every((v) => v === arr[0]) ? [arr[0] as T] : arr
}
