...
<div v-if="champion" class="flex-col w-full gap-6 max-w-inherit h-full pr-[3px]">
    ...
    <Card>
        ...
        <CardContent class='h-full pt-3 overflow-hidden'>

            <!------------------------⟢ tablist ⟣------------------------->
            <div role="tablist" class="tabs tabs-lifted w-[115%] -ml-6">
                <a role="tab" class="tab">Stats

                </a>
                <a role="tab" class="tab tab-active drop-shadow-[2px_-3px_2px_#00000010]">Abilties</a>
                <a role="tab" class="tab pointer-events-none"> 　</a>
                <a role="tab" class="tab pointer-events-none"> 　</a>
            </div>
            <TabListPocket defaultTab="abilities" first="stats"
                class='mt-6 relative  overflow-hidden  flex flex-col flex-1 '>
                <!----------------------⟢ stats ⟣---------------------------->
                <label role="tab" class='tab hidden'></label>
                <Tab v-model:modelName="modelName" value="stats" @click="modelName = 'stats'" first>
                    <template #header>
                ...
                <Tab v-model:modelName="modelName" @click="modelName = 'abilities'" value="abilities"
                    class='ability max-w-inherit w-inherit left-20 '>
                    <template #header>
                    ...
                    <template #content>
                        <ChampionAbilities :champion="champion" class='w-inherit max-w-inherit' />
                    </template>
                </Tab>
...
<style scoped>
.tab {
    --tab-border-color: oklch(var(--b3)/80);
