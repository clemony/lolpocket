  ...
  <TabsIndicator v-bind="{ ...forwarded, ...$attrs }" as-child :class="{ 'h-[81%]': props.orientation != 'vertical', 'w-[98%]': props.orientation == 'vertical' }">
    <motion.div
      ...
      <motion.div
        ...
        :class="cn('   ', { 'shadow-sm shadow-black/3 !bg-b1/90 ': props.orientation == 'vertical', 'bg-neutral/85 border-accent border drop-shadow-sm inset-shadow-sm inset-shadow-b3/20 rounded-xl rounded-lg-2': props.contrast })" />
    </motion.div>
  </TabsIndicator>
</template>
