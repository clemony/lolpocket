...
<div v-if="pocket" :class="{ 'opacity-100': pocket.notes?.length > 0 }" alt="add note" class="relative size-full p-0.5">
    <context-menu>
        <context-menu-trigger>
            <textarea
                class="peer flex size-full cursor-pointer items-start overflow-y-scroll rounded-[4px] border border-transparent bg-transparent focus:border-base-200/40 focus:bg-base-200/25 px-1.5 py-1 align-top text-[10px]  leading-3 tracking-[0.015em] outline-none transition-all duration-300 [resize:none]  placeholder:font-light  hover:border-base-300/70 hover:opacity-70 hover:shadow-inner focus:opacity-100 focus:shadow-inner"
