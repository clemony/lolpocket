...
watch(
  () => as.userAccount.inbox.newMessages,
  (newVal) => {
    if (newVal)
      inbox[1].new = newVal
  },
...
<template>
  <NavigationMenuItem
    ...
    <NavigationMenuTrigger
      :arrow="false"
      class="relative size-11.5 rounded-full grid bg-transparent place-items-center group/acc-btn hover:shadow-sm hover:drop-shadow-sm data-[state=open]:ring-bc/20 hover:ring-bc/20 ring ring-transparent">
    ...
    <NavigationMenuContent
      align="end"
      class="gap-6 pb-2 flex flex-col gap-1 inset-shadow-xxs w-max"
      @focus-outside.prevent>
      <div
        v-if="as?.userAccount?.id"
        class="size-full w-130 bg-b2/40 h-130 flex flex-col">
        <div class="px-4 pt-4 mb-9 flex gap-4 h-14 items-center w-full">
          <SummonerIcon
            class="size-14 shadow-sm drop-shadow-xs rounded-full mt-2" />
          ...
          <div class="flex flex-col space-between size-full">
            <div class="flex gap-1 items-center w-full h-5 relative">
              ...
              <NavigationMenuLink
                ...
                v-tippy="'Admin'"
                class="size-10 grid place-items-center -mr-2 opacity-60 hover:opacity-100 cursor-pointer"
          ...
          <template
            ...
            <LayoutGroup>
              <motion.label
                :value="item.name"
                layout
                class="tab gap-2 z-1 first:-ml-1.25 text-3 n !font-medium flex items-center !border-b-0 group/tab !text-start"
                :class="
                  cn('', { '!pb-[4px] !px-[21px]': item.name != openBox })
                "
                :style="{
                ...
                <icon
                  ...
                  :class="
                    cn(
                      'dst group-hover/tab:opacity-100  transition-opacity duration-200',
                      { 'opacity-50': openBox != item.name },
                      item.icon.class,
                    )
                  " />
                {{ item.name }}
                <AnimatePresence>
                  <motion.span
                    ...
                    :animate="item.new ? 'visible' : 'hidden'"
                    class="ml-2 relative inset-shadow-xxs ring ring-b3 shadow-xs rounded-lg badge-sm text-1">
                    {{ item.new }} new!
                  </motion.span>
                </AnimatePresence>
              </motion.label>
              ...
              <div
                class="tab-content bg-b1 drop-shadow-sm z-0 rounded-none w-full h-[calc(100%-30px)] border-t border-t-b4 pb-10 relative">
                <Inbox
                  v-if="openBox == item.name"
                  :inbox="item.name"
                  @reset-new="item.new == 0" />
              </div>
            </LayoutGroup>
          </template>
        </div>
      ...
      <div
        class="flex border-t border-t-b4/80 pt-2 justify-between items-center px-3">
