...
<template>
  <FormField v-slot="{ componentField }" :name>
    <FormItem :class="cn('relative mt-1 mb-3 pl-3', className)">
      <FormLabel
        v-tippy="{
          content: optional ? 'optional' : null,
          theme: 'neutral',
          placement: 'right',
          offset: [0, 0],
        }"
        class="group/label mb-3 flex w-min cursor-help items-center text-5 font-bold"
      >
        <slot>{{ title }}</slot>
        <Element v-if="optional" size="icon-sm">
          <icon
      ...
      <Textarea :placeholder v-bind="componentField" class="min-h-24 w-full" />
    </FormItem>
  </FormField>
</template>
