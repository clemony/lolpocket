...
<template>
  <DropdownMenu
    ...
    <DropdownMenuTrigger class=" w-full h-10 rounded-lg shadow-sm drop-shadow-sm !bg-n1/85 inset-shadow-sm  items-center font-medium border-accent text-2 grow px-4  **:text-nc justify-start flex group/b">
      ...
      <icon
        name="down-sm"
        class="group-data-[state=open]/b:rotate-180 tldr-20" />
    </DropdownMenuTrigger>

    <ContrastDropdownContent
      :side-offset="2"
      align="start"
      class="flex flex-col w-32 max-h-80 px-1 overflow-y-scroll">
      <ContrastDropdownItem
        v-for="col in filteredCols"
        :key="col.getColId()">
        <label
          v-if="col && col.isVisible()"
          :key="col.getColId() + col.isVisible() + 0"
          class="w-full  group/hide !pointer-events-auto   size-full  text-nowrap !text-2  font-medium px-1 relative  capitalize  flex gap-4 justify-start "
          @click="onColHide(col.getColId())">
          <input
            v-model="hidden"
            class=" hidden"
          ...
          <span
            v-tippy="'hidden'"
            class="size-1.5 grid place-items-center relative justify-self-end *:shrink-0 ">
            <icon
        ...
        <label
          v-else
          :key="col.getColId() + col.isVisible()"
          class="   group/hide !pointer-events-auto px-1  flex-nowrap text-nowrap  !text-2 size-full font-medium  relative  capitalize  flex gap-4 justify-start "
          @click="onColShow(col.getColId())">
          <input
            v-model="hidden"
            class=" hidden"
            :checked="!col.isVisible()"
          ...
          <span
            v-tippy="'hidden'"
            class="size-4.5  relative justify-self-end grid place-items-center *:shrink-0  ">
            <icon
              v-if="col.isVisible() == false"
              name="tick-sm"
