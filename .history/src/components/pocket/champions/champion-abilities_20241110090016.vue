    ...
    <AccordionItem v-for="(ability, key) in champion.abilities" :key="key" :value="key"
        ...
        <AccordionContent>
            ...
            <div v-for="(value, key) in filteredData(
                ability.data)" :key="key" class='flex flex-col gap-2'>
                <div class='flex flex-nowrap w-full bg-base-200/60 rounded-md px-2 py-1'>
                    <div class="text-medium text-sm capitalize">
                        {{ key }}
                    </div>
                    <div class="text-nowrap text-right text-sm grow">
                        {{ value }}
                    </div>
                </div>
            </div>
