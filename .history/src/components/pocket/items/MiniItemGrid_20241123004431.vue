...
<template>
    <template v-for="(item, index) in items" :key="item.id">
        <div
            v-if="index < 6"
            :key="item.id"
            class="first: shrink-0 rounded-lg !rounded-tl-full border border-base-300/70 bg-base-100/50 p-0 shadow-warm backdrop-blur-sm">
            <Item
                :item="item"
                :pocket="pocket"
                imgClass="rounded-lg shadow-sm drop-shadow-sm !p-0 m-0 object-center  size-full border border-base-300"
                class="!size-14 !rounded-full !p-0 ring-0 hover:ring-0"
