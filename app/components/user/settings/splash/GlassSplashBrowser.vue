...
<template>
  <Dialog v-bind="$attrs">
    ...
    >
      ...
      <div
        class="relative flex w-screen max-w-screen flex-col items-center gap-8 overflow-x-hidden overflow-y-auto px-32"
      >
        <Input
          v-model:model-value="searchQuery"
          class="sticky top-0 z-2 mt-44 h-14 w-full max-w-160 shrink-0 rounded-xl bg-b1/90 backdrop-blur"
        >
          <icon name="search" />
          ...
          <template #3>
            <Button
              ...
              variant="ghost"
            >
              <icon name="refresh" class="size-3.75 dst" />
            </Button>
        ...
        <RadioGroup
          ...
          as="div"
          class="mx-auto flex h-max max-h-screen w-fit flex-wrap justify-center gap-8 overflow-x-hidden mask-t-from-50% mask-t-to-96% pt-64"
        >
          <label
          ...
          >
            ...
            <Badge
              variant="neutral"
              size="xl"
              class="absolute bottom-2 left-2 truncate bg-neutral/86 backdrop-blur"
            >
              {{ item.name }}
            </Badge>
            <RadioGroupItem name="item-key" class="hidden" :value="item.key">
            </RadioGroupItem>
          </label>
        </RadioGroup>
      ...
      <DialogClose class="fab pointer-events-auto top-6 right-6 z-20" as-child>
        <Button
          variant="ghost"
          size="12"
          class="border-0 **:text-white hover:bg-b1/80 hover:**:text-bc"
          shape="square"
        >
          <icon name="x" class="size-6 **:stroke-[1.5]" />
        </Button>
      </DialogClose>
    </LazyGlassDialogContent>
  </Dialog>
