...
import type { ContextMenuItem, SelectItem, SelectValue } from "@nuxt/ui"
...
<template>
  <UContextMenu v-bind="forwarded" v-model:open="modelValue" :content :items>
    ...
    <template #select="{ item }">
      <USelect
        ...
        :portal="true"
        :icon="asSelect(item).select.icon"
        :default-open="false"
    ...
    <template #check-trailing="{ item }">
      <Checkbox
        ...
        v-bind="asSwitch(item).indicator"
        :class="cn('rounded-full', asSwitch(item).ui?.itemTrailing)" />
    </template>
  </UContextMenu>
</template>
