...
<template>
  <motion.div
    class="absolute top-0 left-0 z-50 grid size-full max-h-screen w-110 min-w-110! grid-cols-[45px_1fr] items-center rounded-r-xl border-y-1 border-r border-l-0 border-b3/50 bg-b1/90 drop-shadow-md drop-shadow-black/12 backdrop-blur-md"
    @hover-start="
      () => {
        if (!open) open = true;
        hoverStart = true;
  ...
  >
    ...
    <!-- logo -->
    <div
      class="flex h-screen w-full flex-col items-center gap-y-3 border-r border-r-b3/80 pt-3"
    >
      <h1 class="mb-1 dss">LP</h1>
      <BtnLink
        ...
        variant="neutral"
      >
        <PocketIcon :img="pocket.icon" class="size-full scale-120" />
      </BtnLink>
    </div>

    <div
      class="pointer-events-auto relative scrollbar-hidden flex h-screen w-full flex-col overflow-hidden pt-16 pr-2"
    >
      <!-- search buttton -->

      <SearchBox class="5 mr-1 ml-3 h-11 justify-between fx-0 *:first:gap-3" />
      ...
      >
        <Button
          ...
          hover="outline"
          on="btn"
          class="absolute inset-x-0 bottom-3 mx-2 h-16 justify-between border-transparent px-4 backdrop-blur aria-expanded:btn-active aria-expanded:border-b3 aria-expanded:bg-b2/60! aria-expanded:shadow-sm aria-expanded:inset-shadow-sm aria-expanded:inset-shadow-black/4"
        >
