...
const c = b ? {name: `'${props.to}'`, hash: `'${props.hash}'`} : {name: `'${props.to}'`}
    name: '${props.to}', ${hash}: '${props.hash ? props.hash : ''}'
})
...
<template>
    <component :is="components.item">
        <RouterLink :to="" class="flex rounded-lg" activeClass="bg-base-200/70">
