...
<template v-if="set">
    <template v-for="rune in runes1">
        <div
            class="shadow-pretty shrink-0 rounded-full border border-base-300/70 bg-base-100/50 p-1 backdrop-blur-sm">
            <template v-if="rune.name != 'none'">
                <LoadImg
                    :url="rune.img"
                    class="size-[35px] overflow-auto rounded-full shadow-sm ring-1 ring-base-300 drop-shadow-sm" />
            </template>
            <Placeholder
                v-else
                :size="35"
