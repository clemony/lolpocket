...
const {
  ...
  isLoading,
} = throttleFunction(refreshMatches, cooldown)
...
<template>
  <tippy
    ...
    <div
      v-else-if="timeRemaining && timeRemaining?.percent" class="radial-progress opacity-90 !bg-b1 mt-1 mr-1" :style="{
        '--value': timeRemaining?.seconds,
        '--size': '1.25rem',
        '--thickness': '2px',
      }"
      :aria-valuenow="timeRemaining?.percent" role="progressbar" />
    ...
    <template #content>
      <div class="grid    py-1.5 gap-1">
        <p v-if="timeRemaining" class="flex italic !text-2 ">
