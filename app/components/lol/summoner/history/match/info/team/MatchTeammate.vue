...
<template>
  ...
  >
    ...
    <div class="py-3!" :class="divClass">
      <h4
        v-tippy="{
          content: player.riotIdGameName,
          placement: 'top',
        }"
        class="size-full flex-nowrap items-end truncate text-2 font-semibold text-nowrap"
      >
        {{ player.riotIdGameName }}
      </h4>

      <p
        class="flex items-center self-start truncate text-2 font-medium text-nowrap opacity-60"
      >
    ...
    >
      ...
      >
        <Img
          ...
          class="tippy-target absolute size-full rounded-tiny object-center ring-bc/60 transition-all duration-300 *:h-5.5! hover:scale-110 hover:ring"
        />
      </div>
      <div
        v-tippy="{
      ...
      >
        <Img
          ...
          class="absolute size-full rounded-tiny object-center ring-bc/60 transition-all duration-300 *:h-4.5 hover:scale-120 hover:ring"
          :class="{
    ...
    <div class="" :class="divClass">
      <p
        class="flex flex-nowrap items-center text-2! leading-0 font-semibold tracking-wide text-nowrap **:tracking-wide"
      >
      ...
      >
        <icon name="fa6-solid:infinity" class="mt-[0.04em] size-3.5 dst" />
        KDA
      </Badge>

      <p
        v-else
        class="flex gap-1 truncate leading-0 leading-3 font-medium text-nowrap text-bc/80 dst"
      >
        {{ stats.kda / 100 }} KDA
      </p>
    </div>
    ...
    <div :class="cn('relative size-full', divClass)">
      ...
      <p
        class="w-full truncate text-2! leading-4 font-medium tracking-tight text-bc/80"
      >
        {{ Math.round(player.challenges.killParticipation * 100) }}%
        <span class="leading-0 **:text-1!">kp</span>
      </p>
    </div>

    <!-- healing & vision score -->

    <div class="justify-end! justify-items-end! text-nowrap" :class="divClass">
      <p
        v-tippy="{
      ...
      >
        ...
        <span class="relative grid size-3 justify-end">
          <i-lol-hsp name="oi:plus" class="left-0.25 size-3! text-bc/50! dst" />
        </span>
      </p>
    ...
    <div class="justify-end! justify-items-end! text-nowrap" :class="divClass">
      <p :class="cn(pClass)">
        ...
        <span class="relative grid size-3 justify-center">
          <icon name="el:fire" :class="cn('absolute size-3 dst')" />
        </span>
      </p>
    ...
    >
      <template v-for="i in 6" :key="i">
        <Item
          ...
          class="size-8 rounded-md ring-bc/60 transition-all duration-300 *:rounded-md hover:scale-105 hover:ring"
        />

        <Placeholder
          v-else
          class="size-8 shrink-0 rounded-md border-b3 bg-b2/80 shadow-sm shadow-black/4"
        />
      </template>
    </div>
    ...
    <div class="justify-end! justify-items-end! text-nowrap" :class="divClass">
      ...
      <p class="hover:underline">{{ player.goldEarned.toLocaleString() }} G</p>
    </div>
  </div>
</template>
