...
<template>
    <ScrollArea as-child>
        <div
            class="mb-10 flex h-full max-h-64 w-full flex-row flex-wrap gap-2 overflow-y-scroll">
            <label
                v-for="icon in iconStore"
                class="aspect-square size-16 self-center border-transparent p-3 has-[:checked]:bg-base-200/60 has-[:checked]:shadow-standard">
                <input
                    v-if="pocket"
                    type="radio"
                    name="iconPicker"
