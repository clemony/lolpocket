...
import { newPocketDrawer } from './data'
...
const title = computed(() => {
    const t = type.value == 'addPocket' ? newPocketDrawer.title : props.title
    return t
})
const description = computed(() => {
    const d =
        type.value == 'addPocket' ?
            newPocketDrawer.description
        :   props.description
    return d
})
</script>
<template>
    <DrawerRoot :open="gs.drawerState" :key="gs.drawerValue.id">
        <!--     <DrawerTrigger>
            <slot name="trigger" />
        </DrawerTrigger> -->
