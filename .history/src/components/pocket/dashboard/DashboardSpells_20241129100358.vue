...
const spells = ref(props.pocket.spells[0])
...
const spell1 = computed(() => {
    let spell = spells.spell1 ? spells.spell1 : createDefaultSpell()
    return spell as spell
})
const spell2 = computed(() => {
    let spell = spells.spell2 ? spells.spell2 : createDefaultSpell()
    return spell as spell
})
const alt1 = computed(() => {
    let spell = spells.alt1 ? spells.alt1 : createDefaultSpell()
    return spell as spell
})
const alt2 = computed(() => {
    let spell = spells.alt2 ? spells.alt2 : createDefaultSpell()
    return spell as spell
})
...
function update(event, spell) {
    console.log(event)
    const pocket = getPocket(props.pocket.key)
    props.pocket.spells[0].spell1 = event
}
onMounted(async () => {
    await el.value
})
        ...
        <div class="grid grid-cols-[2fr_1fr_1fr] gap-3 px-5 pt-6" ref="el">
            ...
            <div class="items-center justify-self-end">
                <SpellPicker
                    ...
                    :arr="spellArr"
                    @update:model="update($event, spell1)"
                    :alignOffset="-98" />
            </div>
            <div class="items-center justify-self-end">
                <SpellPicker
