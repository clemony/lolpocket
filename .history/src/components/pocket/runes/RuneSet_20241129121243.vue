...
<template v-if="set">
    <template v-for="rune in whichOne">
        <div
            v-if="rune.name != 'none'"
            class="grid place-items-center rounded-full shadow-warm shadow-neutral/40 ring-1 ring-neutral/80">
            <LoadImg :url="rune.img" class="size-11" />
        </div>
        <TextureOutline
            v-else
            class="size-11 rotate-180 bg-base-200/60 p-2 opacity-50 shadow-inner" />
    </template>
</template>
<style scoped></style>
