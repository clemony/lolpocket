...
<template>
    <VueDraggable
        ...
        :fallbackOnBody="true"
        @add="onAdd($event)"
        class="drag-draggable scrollbar-hide h-full flex-nowrap! items-center overflow-x-scroll overflow-y-clip px-0.5 py-1">
        <!-- Display each champion in the draggable list -->
        <div
            v-for="(champion, index) in pocket.champions[0].champions"
            :key="champion.name"
            class="group relative size-[68px] shrink-0">
            <label class="drag-label size-[68px] shrink-0" dragClass="setDrag">
                <div class="drag-wrapper">
                    <img
                        :src="champion.img"
                        class="drag-img scale-[115%]"
                        draggable="false" />
                    <input
                        type="radio"
                        :value="champion"
            ...
            <label
                class="group/star absolute top-1 left-1 z-20 grid h-full w-4 cursor-pointer place-content-center *:transition-all *:duration-300">
                <input
                    type="radio"
                    name="starSet"
