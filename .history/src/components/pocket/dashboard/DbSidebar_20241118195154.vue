    ...
    <div v-if="pocket" :key="pocket.key" class="flex flex-col">
        <Card class="shadow-smooth">
            <CardContent class="h-36 overflow-hidden rounded-lg p-0">
                ...
                <div
                    v-else
                    class="flex size-full flex-wrap items-center justify-center">
                    <p
                        class="size-full items-center whitespace-pre-line text-pretty px-12 text-right align-middle font-serif text-lg italic first-letter:font-bold first-letter:not-italic">
                        {{ formattedQuote() }}
                    </p>
                </div>
            </CardContent>
        ...
        <Tabs default-value="main" class="w-full">
            <TabsList class="mb-2 mt-5 bg-transparent">
                <TabsTrigger value="main">Spells</TabsTrigger>
