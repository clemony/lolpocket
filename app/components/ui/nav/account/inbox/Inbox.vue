...
<template v-if="inbox.length">
    <component :is="inboxType == 'Notifications' ? Notification : Message"   v-for="(item, i) in inbox" :key="i" :item="item"  @clear-notification="inbox.splice(i, 1)" />
</template>

<motion.div  :variants="itemVariants"  class="size-full grid place-items-center" v-else>
