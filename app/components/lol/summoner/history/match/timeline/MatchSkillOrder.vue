    ...
    <Card
      ...
      <div
        ...
        <Img
          ...
          :src="ability.icon" />
        <div
          class="absolute -right-1 -bottom-1 grid size-6.5 place-items-center rounded-full border-2 border-p0 bg-neutral font-mono text-2xs font-semibold text-nc"
          variant="neutral">
          {{ ability.key }}
        </div>
      </div>
    ...
    <div class="flex h-full flex-col justify-evenly gap-1.25 rounded-lg">
      <div
        ...
        <div
          ...
          <div
            class="absolute grid size-7 ring-2 ring-p0"
            :style="{ gridColumnStart: i + 1 }" />
          <div
            v-if="skill === row"
            class="absolute grid size-7 place-items-center bg-neutral text-2xs font-semibold text-nc shadow-sm"
            :style="{ gridColumnStart: i + 1 }">
            {{ i + 1 }}
          </div>
        </div>
