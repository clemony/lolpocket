...
<template>
  <Tabs
    ...
    <IndicatorTabsList
      ...
      group
      :class="cn(
        'min-w-114  max-w-114 overflow-hidden border relative flex-nowrap  size-full  **:select-none', gridCols)">
      <IndicatorTabsTrigger
        v-for="rune in runes"
        :key="rune.id"
        :title="rune.name"
        :value="rune.id"
        :class="cn('rounded-full opacity-75  hover:opacity-100 hover:grayscale-0  place-self-center  group/r border-transparent relative cursor-pointer tldr-30  grid place-items-center shrink-0 size-22 aspect-square',
