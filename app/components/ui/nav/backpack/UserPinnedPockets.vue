...
<template>
  <Field class="relative px-2">
    <icon
      name="pin"
      class="absolute top-3 right-3 size-4.5 dst opacity-80" />

    <ul
      v-if="account && pinned && pinned.length"
      class="space-y-2">
      <li
        ...
        <SheetClose
          ...
          <icon
            name="open"
            class="transition duration-200 size-4" />
        </SheetClose>
      </li>
    </ul>

    <div
      v-else-if="account && !pinned && !pinned.length"
      class="grid place-items-center h-20">
