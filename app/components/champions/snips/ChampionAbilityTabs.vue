...
<template>
  <menu class="flex w-full h-20 items-center px-8 justify-between  gap-2.5 pointer-events-auto z-1">
    <Label
      ...
      :class="{
        'scale-110 hover:scale-115 ': selectedAbility == ability.key, '  drop-shadow-sm  shadow-sm ': loaded,
      }">
      <input
      ...
      <Img
        ...
        :class="{ 'opacity-100 contrast-100 grayscale-0  ': selectedAbility, 'animate-in fade-in duration-500  ': loaded, 'animate-out  duration-500 fade-out-50': loaded }"
        @load="loaded = true" />
