...
<template>
    <VueDraggable ref="el" v-model="set.items" :group="{
        ...
        <template v-for="(item, index) in set.items" :key="item.id">
            <VDropdown v-if="!props.limit || (props.limit && index < 6)" :overflow-padding="20" :shift="true"
        ...
        <template v-if="props.limit">
            <QuickSearch v-for="index in additionalQuickSearchCount" :key="'quicksearch-' + index"
                :array="filteredItems" v-model:quickSearch="quickSearch" v-model:returnData="returnData"
