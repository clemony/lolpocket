export function useMatchSimplifier(puuid: string) {
  ...
  const simplifyMatches = (matches: MatchData[]): SimplifiedMatchData[] => {
    return matches.map(simplifyMatch).filter(Boolean) as SimplifiedMatchData[]
  }
...
/* const { simplifyMatches } = useMatchSimplifier(puuid)

const simplified = simplifyMatches(fullMatches) */
