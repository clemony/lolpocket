...
<div class='flex flex-col gap-1.5 p-1'>
    <VDropdown theme="hoverdd-inner" placement="right-start" :disabled="!ps.pockets.length">
        <Button variant="ghost" size="xs" class='gap-3 items-center'>
            <icon icon="fluent:column-triple-20-regular" class='size-4.5' />
        ...
        <template #popper>
            ...
            <template v-for="column in ps.columns" :key="column.colId">
                ...
                <VDropdown v-if="column.headerName != ''" theme="hidden">
                    <Label class="flex gap-3 text-sm items-center rounded-md hover:bg-base-200/60 w-full py-1 px-2">
        ...
        <icon icon="fluent:mail-inbox-all-20-regular" />
    </VDropdown>
</div>
</template>
