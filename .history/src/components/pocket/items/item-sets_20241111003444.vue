...
<VueDraggable v-if="pocket" tag="div" v-model="pocket.items[0].itemSets" :delay="0" :animation="300"
    ...
    class="z-0 h-full  flex flex-col gap-8">
    <InfoCard v-for="(set, index) in pocket.items[0].itemSets" dragClass="setDrag" :key="set.key" headerClass="pb-2"
        ...
        <template #header>
            ...
            <div class='w-fit -mt-1 -ml-2.5'>
                <InputEditable v-model:modelValue="set.name" :defaultValue="set.name"
                    class='!text-base font-medium text-start items-center align-baseline capitalize  border-transparent focus-within:border-base-200 hover:border-base-200 transition-all duration-300 pr-10'>

                    <div class='flex gap-1 mt-1'>
                        <button>
                            <icon icon="teenyicons:x-outline" class='p-[1px] ' />
                        </button>
                        <button @click.stop="
                            set.name = generateRandomName() + ' Set'">
                            <icon icon="ant-design:question-outlined" class='-mt-1' />
                        </button>
                    </div>
    ...
    <Card
        class='h-[60px] items-center grid justify-center bg-base-200/10 opacity-70 !shadow-inner transition-all duration-500 cursor-pointer hover:opacity-100 hover:!shadow-sm hover:bg-transparent '>
