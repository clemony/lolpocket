...
<template>
 <div class="absolute bg-b1 flex items-center py-1 top-0 left-0 w-full rounded-none border-x-0 shadow-none border-b border-b-b3/80 h-14 px-5.25 gap-2">
...
<input type="color"  class="size-16 -top-2 -left-2 cursor-pointer absolute" v-model="pocket.card.color" @change="console.log(pocket.card.color)"/>
</label>
    ...
    <label v-tippy="'Filter'" class="swap swap-rotate btn  btn-square group btn-ghost ">
      ...
      <Separator orientation="vertical" class="bg-b2 h-5/6 mx-3"/>
        ...
        <button class="btn btn-ghost btn-square control-btn" >
          <icon name="ph:arrows-in-simple-light" class="dst size-5.5"/>
        </button>
        
        <button class="btn btn-ghost btn-square" >
        <icon name="iconoir:zoom-in" class="dst size-5.5"/>
        </button>

        <button class="btn btn-ghost btn-square"  >
          <icon name="iconoir:zoom-out" class="dst size-5.5"/>
        </button>
 </div>
