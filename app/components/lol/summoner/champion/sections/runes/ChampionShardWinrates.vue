    ...
    <div
      ...
      <div
        ...
        <Shard
          :id="shard.id"
          size="c-12"
          :icon-class="{ 'opacity-50': !shards[ix as ShardSlot]?.[shard.id]?.games }"
          ...
          :class="cn('transition-transform duration-200 hover:scale-110', { 'border border-nc!': shards[ix as ShardSlot]?.[shard.id], 'scale-90': !shards[ix as ShardSlot]?.[shard.id] })"
          :variant="shards[ix as ShardSlot]?.[shard.id] ? 'neutral' : 'base'" />
        <WinrateIndicator
          v-if="shards[ix as ShardSlot]?.[shard.id] && shards[ix]?.[shard.id]?.winrate"
