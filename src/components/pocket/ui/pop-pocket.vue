...
<template>
<DialogContent>
    <form ref="form" class="" @submit.prevent.stop="submitForm">
        <DialogHeader>
            <DialogTitle>
                <h1 class="!text-sm tracking-tight text-base-content">
                    {{ title }}
                </h1>
            </DialogTitle>
            <DialogDescription>
        ...
        <div class="flex gap-4 py-4">
            <div class="flex w-1/2 flex-col gap-1">
                ...
                <div class="join relative">
                    <Input type="text" name="pocket-name" v-model="name" placeholder="optional" class="" />

                    <Button variant="ghost" type="button" size="sm" class="group join-item absolute right-0 top-0"
                ...
                <TagsInput v-model="tags" class="flex-col">
                    ...
                    <div class="flex w-full flex-row justify-start gap-2 flex-wrap">
                    ...
                    <TagsInputInput placeholder="optional" class="w-full" name="pocket-tags " />
                </TagsInput>
            </div>
            ...
            <div class="flex w-1/2 flex-col gap-1">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                        <div class="h-48 max-w-56">
                            <IconPicker v-model:selectedIcon="selectedIcon" v-model:bgColor="bgColor"
                ...
                <div class="flex justify-between gap-2">
                    <DropdownMenu class="w-1/2">
                        <DropdownMenuTrigger class="rounded-md data-[state=open]:bg-base-200/60">
                            <Button variant="ghost" size="sm">
                                <span class="size-4 justify-start rounded border border-base-300" :style="{
                                    backgroundColor: bgColor,
                                }"></span>
                                Backdrop
                            </Button>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger class="w-1/2 rounded-md data-[state=open]:bg-base-200/60">
                            <Button variant="ghost" size="sm" class="w-full justify-start">
                                <span class="size-4 rounded border border-base-300" :style="{
        ...
        <DialogFooter>
            <DialogClose>
                <Button v-if="!props.pocketKey" variant="neutral" type="submit"
                    class="btn btn-sm cursor-pointer justify-self-end">
                    {{ submitText }}
                </Button>

                <Button v-else variant="neutral" type="button" class="btn btn-sm cursor-pointer justify-self-end"
                    @click="updatePocket">
                    {{ submitText }}
                </Button>
            </DialogClose>
