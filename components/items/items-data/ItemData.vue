...
const hasNicknames = computed(() => {
  if (!item.value.nickname) return false;
...
<template>
  <div class="**:text-nc max-h-160 overflow-hidden flex flex-col">
...
<template v-if="hasNicknames">
  ...
  <div v-if="effects && effects.pass">
      ...
      <template v-if="recipe && recipeArray">
        <div v-if="item.buy" class="divider divider-end before:h-px my-4 before:bg-nc">
