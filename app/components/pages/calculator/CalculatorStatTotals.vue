...
<template>
  <div class="w-full overflow-hidden">
    <!-- head -->
    <div
      class="btn-depth z-1 grid h-13 w-full grid-cols-[40px_1fr_30px_30px] items-center py-2 *:text-2"
    >
      <div class="col-start-2 font-medium dst">STAT</div>
    ...
    <div class="w-full overflow-y-auto">
      <LayoutGroup>
        <motion.label
          ...
          :key="stat.id"
          :layout="true"
          class="sticky top-0 left-0 z-1 grid cursor-pointer grid-cols-[40px_1fr_30px_30px] items-center border-b border-t-b3/60 border-b-b3/55 bg-b1 py-3 *:justify-start first:border-t last:border-b-b3! hover:border-y hover:border-y-b3/80 hover:bg-b1/50"
        >
          <!-- row 1 -->
    ...
    <div
      class="z-1 grid h-13 w-full grid-cols-[40px_1fr_30px_30px] items-center border-y border-y-b3/50 py-2 *:text-2"
    >
      <div class="ml-4 justify-center">
        <i-ui-gold class="size-6 text-bc/80" />
      </div>

      <div class="col-start-2 font-medium dst">TOTAL GOLD</div>
