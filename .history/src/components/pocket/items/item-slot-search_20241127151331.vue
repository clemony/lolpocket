...
const set = computed(() => {
    return pocket.value.items[0].itemSets.find(
        (set) => set.key == props.thisSet
    )
})
...
function afterLeave(el: HTMLElement) {
    el.style.display = 'none' // Hide after the transition is complete
}
    ...
    <Popover v-model:open="open">
        <PopoverTrigger as-child>
            <button
                ...
                role="combobox"
                :aria-expanded="open"
                class="no-drag search-drop ghosty drag-label group/qs hover:after:text-neutral relative z-0 flex w-[200px] basis-16 cursor-pointer! justify-between p-0! after:absolute after:grid after:h-full after:w-full after:place-content-center after:opacity-50 after:content-['+'] hover:after:opacity-60"
