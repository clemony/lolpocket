...
<template v-if="props.item">
    <div class="relative grid p-4 !min-w-60 max-w-80 w-fit overscroll-none">
        <div
            class="grid grid-cols-[1fr_2fr] border-b border-base-300 pb-3 gap-2 w-full">
 ...
 -->
                    ...
                    <VDropdown
                        ...
                        <template #popper :key="item.name + 'addToSet'">
                            <ul
                                class="!w-full items-center p-2 grid gap-2 min-w-28 pointer-events-auto">
                                <span class="pl-2 cursor-default"
                                    >Add to Set</span
                                >
                                <li
                                    v-for="set in is.itemSets"
                                    class="justify-start btn btn-xs btn-ghost"
                                    @click="
        ...
        <div class="flex items-end pt-2">
            <p
                v-if="item.id"
                class="flex items-end gap-2 text-xs align-baseline grow">
                Item ID: <span class="font-mono">{{ item.id }}</span>
            </p>
            <p
                v-if="item.buy"
                class="flex items-center self-end justify-end gap-1 px-1 text-xs">
