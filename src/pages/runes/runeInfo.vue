    ...
    <div :rune="rune" :key="rune.name" class="h-full ">
      ...
      <label :key="rune.name + '1'"
        ...
        <h2 class="flex items-center justify-start gap-3 sub-text grow">
          ...
          <div v-if="rune">
            <img :src="'/img/runes/' + rune.path + '.webp'" class="object-contain h-5" />
          </div>
        </h2>
      </label>
      ...
      <div id="wrap" :data-tier="rune.tier" class="relative px-4 pt-3" :key="rune.name + '2'">
        <div class="relative z-0 img-wrapper group before:absolute " :href="rune.wiki" target="_blank"
          :title="rune.wiki" :alt="rune.wiki" :key="rune.name + 'img'">
          <div class="link-set">
            <img id="runeImg" :src="rune.img" class="ml-2.5 mt-1 rounded-full float-right" />
