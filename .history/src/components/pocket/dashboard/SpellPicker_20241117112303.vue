...
<template>
    <div v-if="asyncStatus === 'loading'">
        <icon icon="svg-spinners:bouncing-ball" class="size-5" />
    </div>
    <div v-else-if="state.status === 'error'">Oops, an error happened...</div>
    <div v-else-if="state.data">
        <VDropdown v-for="spell in state.data" :key="spell.name">
            <Button variant="outline" size="xs" class="aspect-square size-fit">
                <img
