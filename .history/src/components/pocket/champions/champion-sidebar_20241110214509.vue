    ...
    <Card>
        ...
        <CardHeader class='flex flex-row gap-8 items-center'>
            <div class='relative w-fit'>
                <loadImg class="rounded-full size-20 border-base-300 border shadow-sm aspect-square shrink-0"
                    :url="`/img/champions/${clean(champion.name)}.webp`" />

                <VTooltip theme="tt" placement="bottom"
                    class="absolute -bottom-px -right-[5px] grid size-7 items-center justify-center rounded-full  transition-all duration-500 before: hover:rotate-180"
                    alt="view on Wiki">

                    <a :href="champion.wiki" target="_blank"
                        class="badge relative z-10  flex aspect-square size-7 p-1.5 cursor-alias overflow-hidden rounded-full border-none bg-neutral ring-[3px] ring-neutral-content  text-neutral-content">
            ...
            <div class="flex flex-col w-full h-full *:shrink-0 ">
                <h1 class="font-cursive !not-italic scale-x-110 ml-3 font-bold">
                    {{ champion.name }}
                </h1>
                ...
                <Button variant="outline" size="icon"
                    class="group/liked absolute right-6 top-4  [&_svg]:absolute !apsect-square rounded-full [&_svg]:transition-all [&_svg]:duration-500 [&_svg]:pt-px [&_svg]:size-4.5"
