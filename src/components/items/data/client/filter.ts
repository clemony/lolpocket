...
// Computed filtered items based on your filtering logic
export const filteredItems = computed(() => {
    const ts = useTempStore()
    const as = useAccountStore()
    let filtered = [...items.value] // Create a shallow copy of items for filtering

    // Apply your filtering logic
    if (ts.browseItemStats.length) {
        ts.browseItemStats.forEach((stat) => {
            filtered = filtered.filter((item) =>
                Object.keys(item.stats ? item.stats : '').includes(stat)
            )
        })
    }
    ...
    if (ts.browseItemCats.length) {
        ts.browseItemCats.forEach((cat) => {
            filtered = filtered.filter((item) =>
                Object.keys(item.menu ? item.menu : '').includes(cat)
            )
        })
    }
    ...
    if (ts.browseItemTypes && ts.browseItemTypes !== 'Favorites') {
        filtered = filtered.filter((item) =>
            item.type.includes(ts.browseItemTypes)
        )
    }
    ...
    if (ts.biSearchResult)
        filtered = filtered.filter((item) => ts.biSearchResult.includes(item))
    return filtered
})
