...
export function generateRandomString(): string {
  ...
  const championNames = ds.champions.flatMap((champion) => cleanName(champion.name));

  const itemNames = ds.items.flatMap((item) => cleanName(item.name));

  const moreWords = ['Towers', 'Minions', 'Minion Block', 'Lucker Dog', 'Dongers'];

  const leagueWords = championNames.concat(itemNames).concat(ds.uniqueClass).concat(moreWords);
