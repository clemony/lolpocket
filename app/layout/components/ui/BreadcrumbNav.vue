...
<template>
  <Breadcrumb class="z-11">
    <BreadcrumbList :key="route.fullPath" class="px-3">
      ...
      ></div>
      <template v-else>
        <template v-for="(link, i) in chain" :key="link.name">
          <template v-if="link.meta?.title !== 'Overview'">
            <BreadcrumbSeparator v-if="i !== 0" class="hidden md:block" />
            <BreadcrumbItem class="group hidden md:block">
              <BreadcrumbLink as-child>
                <ULink
                  v-if="!link.meta?.search && link.path !== route.path"
                  :to="String(link.path)"
                  class="text-4 font-medium capitalize transition-all not-group-last-of-type:opacity-60 not-group-last-of-type:hover:opacity-90"
                >
                  {{ link.meta?.title || link.name }}
                </ULink>
                <span
                  v-else
                  class="text-4 font-medium capitalize not-group-last-of-type:opacity-60 not-group-last-of-type:hover:opacity-90"
                >
                  {{
