    ...
    <LazyFadeDialogContent class="bg-transparent min-w-130  w-130 p-0 m-0 min-h-200 max-h-200 shadow-none border-none outline-none focus:outline-none focus:ring-none" @interact-outside="us.commandOpen = false">
      ...
      <motion.div
        ...
        <motion.div class="  backdrop-blur-3xl  inset-shadow-sm size-full relative overflow-auto max-h-200 pb-2">
          <CommandCategory name="Pages" :results="pageResult">
          ...
          <CommandCategory name="Champions" :results="championResult">
            <CommandResult v-for="champion in championResult.slice(0, 3)" :key="champion.key">
          ...
          <CommandCategory name="Items" :results="itemResult">
            <CommandItemResult v-for="item in itemResult.slice(0, 3)" :key="item.id" v-model:active-item="activeItem" :item="item" @update:model-value="e => activeItem = e" />

            <CommandMoreResults v-if="itemResult.length > 3" :results="itemResult">
              <CommandItemResult v-for="item in itemResult.slice(3)" :key="item.id" v-model:active-item="activeItem" :item="item" @update:model-value="e => activeItem = e" />
            </CommandMoreResults>
          </CommandCategory>
        </motion.div>
      </motion.div>
