        ...
        <div class="relative pb-0 pt-1.5">
            <div class="relative flex w-full justify-center px-2 pb-1">
                ...
                <input placeholder="Quick search..." :value="quickSearch" @input="handleInput" spellcheck="false"
                    class="z-0 h-7 w-full select-all rounded-btn bg-transparent px-2  italic !outline-none placeholder: focus:bg-base-200/50 focus:not-italic"
                    autofocus />
            </div>
            ...
            <div v-if="quickSearch"
                ...
                <TransitionGroup name="fade" tag="div" @enter="imageIn" @leave="imageOut">
                    <button v-close-popper v-for="data in props.array" :key="data.id" @click="handleClick(data)"
                        ...
                        <img :src="`/img/${props.type}/${data.id || data.name}.webp`" class="size-4.5 rounded-md" />
                        {{ data.name }}
                    </button>
                </TransitionGroup>
            </div>
