...
<div id="menu" class="w-full h-full" :class="{ minimize: gs.isMinimized == true }" data-tauri-drag-region>
    <div
        ...
        <div class="z-0 flex rounded-full cursor-pointer group/name" :class="{
            'overflow-hidden  !aspect-square bg-neutral text-neutral-content/80 !size-9 ml-[2px] !mb-2 ':
                gs.isMinimized == true,
        }">
        ...
        <button :class="{ 'menu-dropdown-show': showDropdowns[0] }" class="!cursor-pointer menu-dropdown-toggle"
            ...
            <button @click.stop="toggleShow(0)"
                ...
                <icon v-if="showDropdowns[0] == false" icon="teenyicons:down-outline" class="stroke-2 !size-2" />
                <icon v-if="showDropdowns[0] == true" icon="teenyicons:up-outline" class="stroke-2 mt-[2px] !size-2" />
            </button>
        </button>
            ...
            <ul class="overflow-y-scroll max-h-inherit">
                <li class="">
                    <ul :class="{ 'menu-dropdown-show': showDropdowns[0] }"
                        ...
                        <li v-for="pocket in ps.rowData" @click="
                            sn.navigateTo(`/pocket/${pocket.key}/`)
                            ">
                            <button>
                                <icon :icon="pocket.icon" />
                                {{ pocket.name }}
        ...
        <ContextMenu>
            <ContextMenuTrigger>
                <button @click="sn.navigateTo('/pockets/trash')" class="w-full">
                    <div class="grid size-4 overflow-clip place-items-center"
                        :class="{ 'size-5': gs.isMinimized == true }">
                        <icon icon="iconoir:bin-full" class="!size-4.5 -ml-[1px] -mt-[1px]" :class="{
                            'scale-110 mt-0 ml-0':
                                gs.isMinimized == true,
                        }" />
                    </div>
        ...
        <button @click="sn.navigateTo('/pockets/archive')">
            <div class="grid size-4 overflow-clip place-items-center" :class="{ 'size-5': gs.isMinimized == true }">
                <icon icon="fluent:archive-20-regular" class="!size-5 -ml-0.5 -mt-0.5" :class="{
                    'scale-110 mt-0 ml-0': gs.isMinimized == true,
                }" />
            </div>
            <span>Archive</span>
            <span class="grow"></span>
        ...
        <div>
            <button :class="{ 'menu-dropdown-show': showDropdowns[2] }" class="w-full pl-3 menu-dropdown-toggle"
                ...
                <button
                    ...
                    <icon v-if="showDropdowns[2] == false" icon="teenyicons:down-outline" class="stroke-2 !size-2" />
                    <icon v-if="showDropdowns[2] == true" icon="teenyicons:up-outline" class="stroke-2 mt-[2px] !size-2" />
                </button>
            </button>
            ...
            <ul :class="{ 'menu-dropdown-show': showDropdowns[2] }" class="ml-4 menu-dropdown">
                <li>
                    <button @click="sn.navigateTo('/champions/champions')">
                        <Icon icon="teenyicons:user-circle-outline" />
                        <span> Champions</span>
                    </button>
                </li>
                ...
                <li>
                    <button @click="sn.navigateTo('/items')">
                         ...
                         <i-ui-icons-noun-sword class="!size-5 -ml-0.5" />
                        <span> Items</span>
                    </button>
                </li>
                ...
                <li>
                  ...
                  <!--       <icon icon="teenyicons:hexagon-outline" class="" /> -->
