...
<template>
  <menu class="flex w-full h-20 items-center px-8 justify-between  gap-2.5 pointer-events-auto z-1">
    <Label
      v-for="(ability, i) in champAbilities" :key="i" :value="i" class="!p-0 aspect-square size-20 rounded-lg relative bg-b3 group shrink-0 !cursor-pointer **:pointer-events-none  hover:scale-110  transition-transform duration-300 " :class="{
