...
export function createVariants() {
  const vv = {
    hidden: 'hidden',
    base: 'backdrop-blur-sm  bg-b1/90  border-b3',
  ...
  return tv({
    variants: {
      ...
      on: {
        ...
        floating:
          'pointer-events-auto! z-10 cursor-pointer! border-b3 bg-b1/80 shadow-sm shadow-black/4 backdrop-blur-sm fx-0 *:opacity-60 hover:*:opacity-80',
        ghost: 'on:border-b3/40! on:bg-b2/40! on:fx-1',
        inset:
          'on:border! on:border-b3/80! on:bg-b2! on:inset-shadow-xs on:fx-1',
