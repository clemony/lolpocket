...
const { arrivedState, directions } = useScroll(scrollArea)

const isTop = ref(arrivedState.top)
console.log(isTop.value)
</script>

<template>
    <Card :class="cn('relative h-fit w-full pt-0', props.cardClass)" ref="card">
        <CardHeader
            :class="
                cn(
                    'absolute z-20 w-full bg-base-100/90 px-8 backdrop-blur-md',
                    { 'py-5': isTop },
                    props.headerClass
                )
            ">
