...
Worker deploy flow after both D1 bindings are configured and after applying
`migrations/0003_summoner_match_scan_state.sql` and
`migrations/0004_summoner_match_gather_control.sql` to `SUMMONER_CACHE_DB`.
Do not use this cron-enabled Worker build as the Cloudflare Pages build.

```bash
pnpm worker:deploy
```

The gatherer uses the existing Riot request queue. Scheduled runs pass
...
For the cron-enabled NA analytics Worker, use `pnpm worker:deploy` instead.
That script first runs the `MATCH_ANALYTICS_CRON=1` Worker build, then deploys
the generated `.output` artifact with Wrangler.
