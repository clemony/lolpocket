<template>
  <KeepAlive>
    <div id="main-runes">
      <div class="rune-wrapper sm:px-6 sm:py-0 md:gap-12 lg:grid-cols-2 xl:grid-cols-2">
        <div class="rune-panel md:gap-2.4  lg:col-span-1">
          <Tabs default-value="precision">
            <div class="flex items-center">
              <TabsList class="rune-tabs">
                <TabsTrigger v-for="tab in tabs1" :key="tab.value" :value="tab.value"
            ...
            <div class="mt-4 rune-tab-content">
              <TabsContent v-for="tab in tabs1" :key="tab.value" :value="tab.value">
                <Card>
                  <CardHeader>
                  ...
                  <CardContent class="rune-content">
                    ...
                    <RadioGroup class="rune-grid">
                      <div v-for="rune in filteredKeystoneRunes" :key="rune.name">
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger as-child>
                                <img class="rune-key-img" :src="rune.img" :alt="rune.name"
                                  @click="handleRuneClick(rune.name)"
                                  :class="{ 'animate-bounce': activeRune === rune.name }" />
                              </TooltipTrigger>

                              <TooltipContent align="start" side="bottom">
                                <button @click="handleTipClick(rune)" class="flex align-middle sheet-trigger-btn">
                                  {{ rune.name }}
                                  <PanelRightClose stroke-width="2" :absolute-stroke-width="true" :size="13"
                                    class="ml-1 mt-0.5" />
        ...
        <div class="rune-panel md:gap-2.4 lg:col-span-1">
          ...
          <Tabs :default-value="defaultTab" v-model:modelValue="selectedTab">
            <div class="flex items-center">
              <TabsList class="rune-tabs">
                <TransitionGroup name="rt2" tag="button">
                  <TabsTrigger v-for="tab in tabs2" :key="tab.value" :value="tab.value" class="rune-2">
                    <img class="i-rune" :src="tab.img" :alt="tab.name" />
                  </TabsTrigger>
                </TransitionGroup>
              </TabsList>
            ...
            <div class="mt-4 rune-tab-content">
              <TabsContent v-for="tab in tabs2" :key="tab.id" :value="tab.value">
                <Card>
                  <CardHeader>
                    <CardTitle class="ct-rune">
  ...
  <Sheet :open="sheetOpen" @update:open="sheetOpen = $event" :rune="clickedRune">
    <SheetContent>
      <SheetHeader>
...
const tabs1: Tab[] = [
  { id: 1, name: "precision", value: "precision", img: new URL("/img/runes/precision.webp", import.meta.url).href },
  { id: 2, name: "domination", value: "domination", img: new URL("/img/runes/domination.webp", import.meta.url).href },
  { id: 3, name: "sorcery", value: "sorcery", img: new URL("/img/runes/sorcery.webp", import.meta.url).href },
  { id: 4, name: "resolve", value: "resolve", img: new URL("/img/runes/resolve.webp", import.meta.url).href },
  { id: 5, name: "inspiration", value: "inspiration", img: new URL("/img/runes/inspiration.webp", import.meta.url).href },
];
...
const filterRunes = (path: string, tier: number): Rune[] => {
  return Runes.value.filter((rune: Rune) => rune.path.toLowerCase() === path.toLowerCase() && rune.tier === tier);
};
