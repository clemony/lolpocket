...
<template>
    <div
        class="scroll-none shadow-cham border-b3 bg-b1/90 mx-0 flex h-full w-full flex-wrap gap-4 overflow-scroll rounded-xl border p-4 backdrop-blur-md">
        <label
            :for="champion.name"
            v-for="(champion, index) in champions"
            :key="champion.name"
            :data-index="index"
            class="item-wrapper__item ring-b3 ring-offset-b1 hover:ring-offset-primary has-checked:ring-offset-secondary flex aspect-square w-[65px] max-w-[70px] min-w-[60px] grow place-items-center overflow-hidden rounded-full bg-cover ring-1 ring-offset-1 drop-shadow-sm has-checked:ring-offset-2"
