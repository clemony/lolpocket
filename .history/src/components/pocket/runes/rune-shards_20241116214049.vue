...
const set = ref(rs.selectedRuneSetIndex)
...
function updateShards(slot, shard) {
    ...
    slot == 1 ?
        ((shard1.value = shard),
        (pocket.value.runes[0].runeSets[set.value].shards = shard1.value))
    : slot == 2 ?
        ((shard2.value = shard),
        (pocket.value.runes[0].runeSets[set.value].shards = shard2.value))
    : slot == 3 ?
        ((shard3.value = shard),
        (pocket.value.runes[0].runeSets[set.value].shards = shard3.value))
    :   ''
}
</script>
        ...
        <div
            ...
            <label
                ...
                <input
                    ...
                    v-model="shard1"
                    class="peer hidden"
                    @change="updateShards(shard.slotID, shard)" />
                ...
                <div
                    ...
                    <Icon
                        :icon="shard.icon"
                        class="size-6 text-base-content transition-all duration-300" />
                </div>
            </label>
        </div>
    </div>
