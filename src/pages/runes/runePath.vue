...
<template>
  <div class="w-full z-0 bg-clip-border transition-all duration-500">
    <div class="flex h-16 items-end [&_img]:h-[1.5rem] z-10 relative after:absolute after:w-full after:h-16 after:top-0 after:left-0 after:rounded-t-box">
      <label
        v-if="data"
        ...
        <input
          class="hidden peer"
          type="radio"
          v-model="rs.selectedPaths[props.selectIdentifier]"
          :name="props.selectIdentifier"
          :aria-label="path"
          :value="path"
        ...
        <div
          class="grayscale brightness-75 peer-checked:brightness-100 peer-checked:grayscale-0 hover:brightness-100 drop-shadow-sm hover:grayscale-0 size-10 aspect-square mx-2 flex justify-center items-center">
          <img
            :src="'/img/runes/' + path + '.webp' || '/img/ui/RunesIcon.svg'"
            class="drop-shadow-sm" />
        </div>
        <div class="capitalize opacity-0 w-0 peer-checked:w-32 peer-checked:opacity-100 transition-all duration-500 font-light mt-1.5 text-lg">
          {{ path }}
        </div>
      </label>
    </div>
