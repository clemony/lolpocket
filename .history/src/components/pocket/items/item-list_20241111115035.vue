...
<VueDraggable ref="el" :group="{ name: 'items', pull: 'clone', put: false, revertClone: true }" :sort="false"
    ...
    class="drag-draggable content-start items-start justify-around  py-4 pr-6 scrollbar-hide h-full max-h-full">
    <TransitionGroup name="zoom" :style="{ '--total': sfi.length }">

        <template v-for="(item, index) in sfi" :key="item.id" :index="index" :style="{'--index': index}">
    ...
    <div class="aspect-square size-[52px]"></div>
    <div class="aspect-square size-[52px]"></div>
    <div class="aspect-square size-[52px]"></div>
    <div class="aspect-square size-[52px]"></div>
    <div class="aspect-square size-[52px]"></div>
...
.zoom-leave-active {
    ...
    position: absolute;
    animation-delay: calc(0.2s * (var(--total) - var(--i)));
}
</style>
