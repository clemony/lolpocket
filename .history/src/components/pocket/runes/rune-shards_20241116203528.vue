...
function updateShards(slot, shard) {
    slot == 1 ?
        ((shard1.value = shard),
        (pocket.value.runes[0].runeSets[model].shards = shard1.value))
    : slot == 2 ?
        ((shard2.value = shard),
        (pocket.value.runes[0].runeSets[model].shards = shard2.value))
    : slot == 3 ?
        ((shard3.value = shard),
        (pocket.value.runes[0].runeSets[model].shards = shard3.value))
    :   ''
}
</script>
