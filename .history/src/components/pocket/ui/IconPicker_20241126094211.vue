...
<template>
    <ScrollArea
        class="mb-6 flex h-full max-h-64 w-2/4 flex-wrap gap-1 overflow-y-scroll">
        <Label
            variant="ghost"
            size="icon"
            v-for="icon in iconStore"
            class="aspect-square size-12 self-center border-transparent p-2 has-[:checked]:bg-base-200/60 has-[:checked]:shadow-standard">
            <input
                v-if="pocket"
                type="radio"
                name="iconPicker"
