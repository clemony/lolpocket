...
export const abilityVariants = tv({
  ...
  slots: {
    ...
    //
    attributeBadge:
      "bg-b2/10 badge border-b3 !text-2 bg-b1 **:text-bc z-10 -mt-1 inline items-center gap-0 border px-2 text-wrap whitespace-pre",
    attributeContent:
      "isolation-auto !m-0 inline w-full gap-3 align-middle leading-7.5",
    attributeWrapper: "mb-3 flex w-full flex-col gap-2 !p-0",
    description:
      "group/d text-3 flex flex-col items-start justify-start leading-6 **:leading-6 *:last-of-type:mb-5",
  },
  variants: {
    color: {
      base: {
        attributeContent:
          "chat-bubble drop-shadow-black/4 bg-b1  drop-shadow-xs",
