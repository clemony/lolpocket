...
<div class="h-full w-full *:scrollbar-hide">
    <div class="flex w-full items-end p-4">
        <div class="flex grow items-center">
            <h2 class="p-1.5 font-size-8 font-bold">Things you Like</h2>
        </div>
    </div>
    ...
    <Splitpanes id="likeSplit" class="default-theme h-[calc(100%-100px)] max-h-[calc(100%-100px)] w-full px-6 pb-2">
        <Pane size="50" max-size="70" min-size="30"
            class="shadow-warm relative h-full rounded-box border border-b3 bg-b1/90! px-4">
            <div
        ...
        <Pane size="50" max-size="70" min-size="30" class="relative h-[inherit] rounded-box border border-b3">
            <div
                class="shadow-warm absolute left-0 top-0 z-10 flex h-fit! w-full flex-wrap items-center justify-end gap-4 border-b border-b-b3 bg-b1/80 px-4 py-1 backdrop-blur-md">
                    ...
                    <label class="drag-label overflow-hidden!">
                        <div class="drag-wrapper overflow-hidden!">
                            ...
                            <div :key="item.id + 'Count'" :class="{ 'opacity-85!': item.count > 1 }"
                                class="shadow-warm absolute -right-1.5 -top-1.5 z-30 flex size-6 place-content-center place-items-center overflow-hidden rounded-full bg-primary font-mono font-size-2 text-primary-content opacity-0">
                                {{ item.count }}
                            </div>
                        </div>
                    </label>
