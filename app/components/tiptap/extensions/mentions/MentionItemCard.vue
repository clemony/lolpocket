  ...
  >
    ...
    <div class="relative grid auto-rows-auto overflow-y-auto px-4 pb-2">
      ...
      <template v-if="item.passives?.length && item.noEffects !== true">
        <Separator :size="2" color="neutral" />
        <ItemEffect
      ...
      <template v-if="item.active?.[0] && item.noEffects !== true">
        <Separator :size="2" color="neutral" />
        <ItemEffect :data="item.active[0]" type="Active" />
      </template>

      <!-- RECIPE -->

      <template v-if="item.buildsFrom?.length">
        <Separator :size="4" label="RECIPE" color="neutral" />
        <div class="group flex items-center gap-3 p-1">
          <template v-for="(fromItem, i) in item.buildsFrom" :key="i">
            <Item
              :id="fromItem.id"
          ...
          <div v-if="item.shop?.prices?.combined" class="flex items-center">
            <icon name="dashicons:plus" class="size-3.5 opacity-80" />
      ...
      <template v-if="item.buildsInto?.length">
        <Separator :size="4" label="BUILDS INTO" color="neutral" />
