...
<div v-if="champion" class="flex-col w-full gap-6">
    ...
    <div class='mt-8'>
        ...
        <TabListPocket :modelName="cTabs" :defaultTab="2" class='!tabs-lg' <!----------------------⟢ stats
            ⟣---------------------------->
            ...
            <Tab v-for="(ability, key) in champion.abilities" :key="key" :value="key" class='!p-2 h-fit'>
                <template #header>
                    <input type="radio" class="peer hidden" name="cTabs" value="stats" v-model="cTabs" />

                    <loadImg :url="ability.img" :alt="ability.name" class="shadow-sm rounded-lg size-10" />
            ...
            <Tab :value="2">
                ...
                <template #content>
                    <div
                        class="ability-wrapper join join-vertical grid w-full grid-cols-1 overflow-y-scroll !border-none">
            ...
            <Tab :value="3">
                ...
                <template #content>
                    ...
                    <div class="drag-draggable !justify-start !gap-3 overflow-y-scroll !p-1 scrollbar-hide">
                        <VDropdown v-for="item in cs.getChampionItems(champion.name)" :key="item.id"
                            :overflow-padding="20" :shift="true" theme="default" :distance="6" @click.right.prevent=""
                            :ref="item.name" class="relative aspect-square !max-h-14 basis-14">
