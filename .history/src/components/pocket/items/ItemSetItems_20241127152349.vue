...
<template>
    <VueDraggable
        ...
        :fallbackOnBody="true"
        @end="handleDragEnd"
        class="drag-draggable item scrollbar-hide justify-start! overflow-hidden p-1!"
                ...
                <label class="drag-label overflow-hidden!">
                    <div class="drag-wrapper overflow-hidden!">
                        ...
                        <!--   <div :key="item.id + 'Count'" :class="{ 'opacity-85!': item.count > 1 }"
                            class="absolute rounded-full bg-primary text-primary-content z-30 -right-1.5 -top-1.5 size-6 flex place-items-center place-content-center font-mono opacity-0 text-2 shadow-warm overflow-hidden">
                            {{ item.count }}
                        </div> -->
                    </div>
                </label>
