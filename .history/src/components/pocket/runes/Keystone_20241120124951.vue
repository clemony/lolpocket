...
<template>
    <template v-if="set">
        <div class="relative z-0 shrink-0 self-center opacity-90">
            <img
            ...
            <div
                v-if="set.secondary != ''"
                class="absolute bottom-0 right-0 size-9 rounded-full bg-base-100/80 shadow-standard backdrop-blur-md">
                <img :src="`/img/runes/${set.secondary}.webp`" class="size-8" />
            </div>
        </div>
    </template>
</template>
