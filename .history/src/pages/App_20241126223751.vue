...
<template>
    ...
    <MainMenubar :class="{ 'isolate !z-0': state == 'open' }" class="menubar" />
    <main
        class="wrapper size-full"
        :key="state"
        :class="{
            'backdrop-brightness-50 before:absolute before:left-0 before:top-0 before:h-screen before:w-screen before:bg-black/30 before:transition-all before:duration-500 before:animate-in before:fade-in-0':
                state == 'open',
        }">
        <div
            v-if="state == 'open'"
            class="command isolate z-[1000] h-1/3 animate-in fade-in-0 slide-in-from-bottom-8">
            <Command />
        </div>
        <label>
            <input
            ...
            <SidebarProvider
                ...
                :class="{
                    'scale-75 overflow-hidden rounded-xl bg-base-100/95':
                        state == 'open',
                }">
                <MainMenubar
