...
<template>
    <div v-if="pocket" :class="{ 'opacity-100': pocket.notes?.length > 0 }" alt="add note"
        ...
        <context-menu>
            <context-menu-trigger>
                ...
                <textarea
                    ...
                    v-model="notesInput" @blur="updateNotes" @click.stop placeholder="">

      </textarea>


                <icon icon="radix-icons:text" class='size-3.5 opacity-30 absolute bottom-1.5 right-1.5'
                    :class="{ 'peer-focus:opacity-0': notesInput != '　' && notesInput != '' }" />
                <icon :class="{ 'peer-focus:opacity-40': notesInput != '　' && notesInput != '' }"
                    icon="radix-icons:text-none" class='size-3.5 opacity-0 absolute bottom-1.5 right-1.5'
