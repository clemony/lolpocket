...
<div v-if="champion" class="flex-col w-full gap-6 max-w-inherit h-full pr-[3px]">
    ...
    <Card>
        ...
        <CardContent class='h-full pt-3 overflow-hidden'>
            ...
            <!------------------------⟢ tablist ⟣------------------------->
            <div role="tablist" class="tabs tabs-lifted w-[115%] -ml-7">
                <label role="tab" class="tab flex items-center gap-2"
                    :class="{ 'tab-active drop-shadow-[2px_-2px_1px_#00000005]': tabModel == 'stats' }">
                    <icon icon="mdi:bar-chart" class='-ml-2' />
                    Stats

                    <input type='radio' name='tabModel' v-model="tabModel" value="stats" class='hidden' />
                </label>
                <label role="tab" class="tab flex items-center gap-2"
                    :class="{ 'tab-active drop-shadow-[2px_-2px_1px_#00000005]': tabModel == 'abilities' }">
                    <icon icon="iconoir:magic-wand" class='-ml-2 size-4' />
                    Abilties
