...
<template>
    ...
    <div v-else-if="state.data">
        <Popover>
            <PopoverTrigger class="grid items-center justify-items-end gap-3">
                <Placeholder class="size-16 rounded-lg" />
            </PopoverTrigger>
            <PopoverContent
                class="-ml-[28px] w-[300px] overflow-hidden !rounded-box bg-transparent p-0 pb-3 backdrop-blur-none"
                :sideOffset="-80">
                <div class="h-28" />
                <div
                    class="gap flex flex-wrap justify-evenly gap-x-5 gap-y-4 bg-base-100 px-14 py-6 backdrop-blur-md">
                    <VTooltip
                        theme="tt"
                        placement="bottom"
                        :distance="6"
