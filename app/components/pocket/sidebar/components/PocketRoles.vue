  ...
  <Collapsible
    ...
    <CollapsibleContent
      ...
      <transition-slide
        ...
        group>
        <template
          v-for="role in mapPositions"
          :key="role">
          <Label
            v-if="role"
            class="order-3 rounded-lg border-p3/80 bg-p0"
            variant="outline"
            size="8"
            ...
            <component
              ...
              class="h-3.75 w-auto shrink-0 text-pc/70 dst peer-checked:text-nc"
              :class="{ 'text-pc/80!': role.name.toLowerCase() === 'all' }" />
            {{ role.name }}
          </Label>
        </template>
      </transition-slide>
