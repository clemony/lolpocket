...
<template>
  <div
    class="ease  border-b3 shadow-smooth relative size-full h-fit w-120 max-w-120 rounded-xl  transition-all duration-500 **:select-none"
    :data-path="selectedPath"
    :class="selectedPath.toLowerCase()"
  >
    <div class="bg-b2/70 backdrop-blur-md flex size-full  flex-col justify-center gap-16 justify-self-center rounded-xl  py-12">
      <TransitionGroup

        enter-active-class="transition-[transform,opacity] duration-500"
        move-class="transition-all duration-500"
        enter-from-class="scale-0 opacity-0"
        enter-to-class="opacity-100 scale-100"
        leave-active-class="hidden duration-0 absolute"
          ...
          >
            ...
            >
              ...
              <img
                ...
                :alt="rune.name"
                class="size-16"
                :class="{ 'absolute  opacity-0 group-hover/r:opacity-100 duration-300 transition-all peer-checked:opacity-100 drop-shadow-sm !size-22': i == 0 }"
              />
              <img
                v-if="i == 0"
                :src="`/img/runes/${selectedPath}/${rune.name.replace(/\s/g, '')}_grayscale.webp`"
                :alt="rune.name"
                class="size-22 absolute opacity-100 drop-shadow-sm group-hover/r:opacity-0 duration-300 transition-all peer-checked:opacity-0"
              />
            </label>
          </div>
        </div>
