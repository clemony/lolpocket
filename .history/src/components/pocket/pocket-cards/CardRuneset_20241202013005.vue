...
<template>
    <div
        :class="
            cn(
                'absolute z-10 h-56 w-12 flex-col items-center justify-end space-y-4 rounded-lg',
                props.class
            )
        ">
        <template v-for="rune in runes1.concat(runes2)">
            <div
                ...
                :class="{
                    'border border-neutral/80 shadow-warm shadow-neutral/40 ring-1 ring-transparent ring-offset-2 ring-offset-base-100/60':
                        rune.name != 'none',
                }">
                <LoadImg
                    v-if="rune.name != 'none'"
                ...
                <ShadowedPlaceholder v-else class="size-9 rounded-full" />
            </div>
        </template>
    </div>
</template>
