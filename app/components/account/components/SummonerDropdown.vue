...
<template>
  <DropdownMenu class="">
    <DropdownMenuTrigger :class="cn('h-full max-w-76 w-full grow self-end inline-flex btn btn-ghost  btn-shadow-ghost justify-start !h-14  items-center gap-3', className)">
      <LazySummonerIcon
        v-if="summoner"
        :summoner
    ...
    <LazyDropdownMenuPopContent>
      ...
      <DropdownMenuItem
        ...
        <icon
          name="at"
          class="**:stroke-[2.1]" />
        <span class="grow font-medium opacity-60">
          Server Region:
        </span>
        <SummonerRegion
      ...
      <div class="grid grid-cols-4 *:btn-shadow *:btn *:not-hover:!bg-b1 *:not-hover:opacity-70 *:not-hover:**:opacity-90 gap-1.5 *:place-items-center *:grid *:[&_svg]:size-5.5 *:size-full h-14 p-1">
        <DropdownMenuItem
          class=""
          as-child>
          <FollowButton
