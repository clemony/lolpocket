...
const onHeaderReady = (params) => {
    ...
    const cols = headerApi.value.getAllColumns();

    columns.value = cols.map(col => ({
        headerName: col.getColDef().headerName,
        isVisible: col.isVisible(), // Get current visibility status
        colId: col.getColId() // Store colId to toggle visibility
    }));
}
...
const toggleColumnVisibility = (colId, isVisible) => {
    headerApi.value.setColumnsVisible([colId], isVisible);

    // Update visibility status in the columns array
    const columnToUpdate = columns.value.find(col => col.colId === colId);
    if (columnToUpdate) {
...
<template>
<div class='w-full top-[65px] h-10 bg-base-100/80 backdrop-blur-md absolute z-10'>
...
<ResizablePanelGroup direction="vertical" class='h-full ' auto-save-id="gs.pocketGridSize">
    ...
    <ResizablePanel :minSize="8" collapsible :collapsed-size="3.5">
        ...
        <ag-grid-vue :theme="pocketTheme" class="top-grid h-full  items-center !rounded-none !bg-transparent "
            ref="pinnedGrid" @grid-ready="onPinnedReady" :columnDefs="columnDefs" :gridOptions="pinnedOptions"
            :rowData="pinnedData" @selection-changed="onSelectionChanged" :getRowId="getRowId" :rowClass="topRowClass"
            ...
            :quickFilterMatcher="quickFilterMatcher" :pinnedTopRowData="pinnedPinnedTopRowData"
    ...
    <ResizableHandle
        class='!h-8 w-full !bg-transparent !border-transparent ring-0 !shadow-none !shadow-transparent ![color:transparent]'>
    ...
    <ResizablePanel :defaultSize='80' collapsible :collapsed-size="5" :minSize="20">
        <ag-grid-vue :theme="pocketTheme" class="bottom-grid h-[93%] items-center !rounded-none !bg-transparent "
            ...
            :defaultColDef="defaultColDef" :suppressHorizontalScroll="false" :autoSizeStrategy="autoSizeStrategy"
            :cacheQuickFilter="true" :quickFilterParser="quickFilterParser" :quickFilterMatcher="quickFilterMatcher"
            :pinnedTopRowData="pinnedTopRowData" :fullWidthCellRenderer="TableHeader"
