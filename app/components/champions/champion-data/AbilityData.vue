...
const ability = computed (() => Object.values(abilities[selected.value])[0] as Ability)
watch(() => ability.value, (newVal) => {
  ...
  <div class="flex flex-col pt-6 **:text-3 gap-6 size-full max-h-full items-center">
    ...
    <div class="w-full flex flex-wrap items-center px-9 gap-x-8 gap-y-4 **:font-medium pr-1" v-if="ability && ( ability.resource || ability.cooldown?.length || ability.rechargeRate || ability.cost?.length || ability.effectRadius || ability.targetRange)">
      ...
      <template v-if="ability.resource == 'Charge'">
        ...
        <div v-if="ability.rechargeRate" v-tippy="'Recharge Rate'" class="flex gap-2 items-center   ">
      ...
      <div v-else-if="ability.cost?.length" v-tippy="`${ability.resource} Cost`" :name="ability.resource" class="flex gap-2   items-center">
        <ChampionDataIcon v-if="ability.resource" :name="ability.resource" class="dst size-4   text-bc/80" />

        <ValueFormatter :array="ability.cost" />
      </div>

      <div v-if="ability.effectRadius" v-tippy="'Effect Radius'" class="flex gap-2 items-center  ">
      ...
      <div v-if="ability.targetRange" v-tippy="'Range'" class="flex gap-2 items-center  ">
