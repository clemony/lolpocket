...
function handleLike(thisChamp) {
    ...
    if (likedChamps.some((champ) => champ.name === thisChamp.name)) {
        const index = likedChamps.findIndex(
...
<template>
    <div
        ...
        <div
            ...
            <div class="grid grid-cols-3 col-span-2 gap-1 p-1">
                <div
                    ...
                    <div
                        class="z-0 grid object-contain w-16 h-16 my-auto rounded-full justify-content-center">
                        <svg role="none" class="z-10 w-full h-full">
                            ...
                            <g mask="url(#circle)">
                                <image
                                    ...
                                    width="115%"
                                    class="max-w-[115%]"
                                    :href="`/img/champions/${champion.name.replace(/\s/g, '').replace(/\'/g, '').replace(/\./g, '').replace(/\&/g, '')}.webp`"></image>
                                <circle
                                    fill="none"
                                    cx="60"
                                    cy="60"
                ...
                <div
                    class="relative col-span-2 col-start-2 -mt-1 -ml-1 antialiased">
                    <h1
                        class="text-xl tracking-tight mb-0.5 pb-0 antialiased font-semibold">
                        {{ champion.name }}
                    </h1>
        ...
        <div
            ...
            <label
                role="tab"
                :class="{ 'tab-active': cs.champTabs == 'stats' }"
                class="tab first">
                <input
                    type="radio"
                    class="hidden peer"
                    name="cTabs"
                    v-model="cs.champTabs"
                    value="stats" />
                Stats
            </label>
            ...
            <label
                role="tab"
                :class="{ 'tab-active': cs.champTabs == 'abilities' }"
                class="!z-20 tab">
                <input
                    type="radio"
                    class="hidden peer"
            ...
            <div
                ...
                <div
                    class="grid w-full grid-cols-1 overflow-y-scroll ability-wrapper join join-vertical !border-none">
                    <div
                        ...
                        <div class="h-auto overflow-y-auto collapse-content">
                            ...
                            <p
                                class="h-auto p-1 overflow-y-scroll text-xs whitespace-pre-line text-balance">
                                {{ ability.context }}
                            </p>
                        </div>
                    </div>
            ...
            <label
                role="tab"
                :class="{ 'tab-active': cs.champTabs == 'items' }"
                class="w-full tab last">
                <input
                    type="radio"
                    class="hidden peer"
                    name="cTabs"
            ...
            <div
                ...
                <div class="w-full px-2 pt-1 pb-3 font-medium">
                    According to you, {{ champion.name }} likes...
                </div>

                <div
                    class="!p-1 drag-draggable !gap-3 !justify-start scrollbar-hide overflow-y-scroll">
                    <VDropdown
                        v-for="item in cs.getChampionItems(champion.name)"
                        :key="item.id"
                        :overflow-padding="20"
                        :shift="true"
                        theme="detail"
                    ...
                    <VDropdown
                        ...
                        alt="Quick Search"
                        :distance="8"
                        class="search-drop ghosty drag-label flex overflow-clip !aspect-square !max-h-14 basis-14 h-auto !p-0 group/qs after:grid after:place-content-center after:w-full after:h-full after:content-['+'] after:absolute relative hover:after:text-neutral z-0 hover:after:opacity-60 after:opacity-50 !cursor-pointer">
                        <div
                            class="group-hover/qs:opacity-40 group-hover/qs:scale-95 scale-105 z-20 opacity-0 select-none bg-[url('/img/ui/frame.webp')] bg-center bg-contain transition-all bg-no-repeat duration-200 w-full h-full brightness-0 cursor-pointer"></div>
