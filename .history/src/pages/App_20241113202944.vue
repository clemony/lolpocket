...
function toggleState(the) {
    ...
    if (state.value == false) {
        ...
        submitText.value = the.submitText || ''
        emit('update:open')
    }
}
...
<template>
    <Dialog
        ...
        <DialogTrigger
            @click="toggleState(newPocketModal)"
            v-shortkey.once="['meta', 'p']"
            @shortkey="toggleState(newPocketModal)"
            class="absolute right-1/4 z-[2000]">
            click
        </DialogTrigger>
        <DialogContent>
            <component :key="id" :is="component" />
        </DialogContent>
        ...
        <SidebarProvider>
            ...
            <SidebarInset class="relative">
                <div
                    class="absolute flex h-full w-full min-w-full overflow-scroll">
                    <NavTabs />
                </div>
                <div
