...
<template>
  <Popover v-model:open="open">
    ...
    <LazyPopoverContent
      ...
      :side-offset="4">
      <div
        class="grid h-16 w-full shrink-0 grid-cols-4 gap-1.5 bg-p2/30 px-2.5 pt-2.5 pb-3 bg-blend-screen">
        <Button
      ...
      <!-- separator  -->
      <DropdownMenuSeparator class="-mt-2 mb-0 bg-p3/40" />
      <article class="size-full overflow-y-auto">
        <section class="z-auto">
          <div
            class="sticky top-0 z-1 w-full bg-p0/30 bg-blend-darken backdrop-blur-sm">
            ...
            <DropdownMenuSeparator class="mt-0 mb-2 bg-p3/40" />
          </div>
          <!-- pinned -->
          <template
            v-for="pock in ps()
          ...
          <!-- separator  -->
          <DropdownMenuSeparator class="mb-0 bg-p3/40" />
        </section>
