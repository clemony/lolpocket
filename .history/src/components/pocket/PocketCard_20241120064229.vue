...
const champImage = computed(() => {
    return `bg-[url(\'/img/champions/splash/${clean(pocket.value.champions[0].starred)}_0.webp\')]`
})
...
<template>
    <label
        class="swap swap-flip relative h-[320px] w-[250px] bg-base-100/95 backdrop-blur-md">
        <input type="checkbox" />

        <Card
            class="swap-off h-[320px] w-[250px] max-w-[250px] overflow-hidden rounded-box bg-base-100/95 bg-[20%_-125%] bg-no-repeat [background-size:200%]"
            :class="{ champImage }">
            <CardContent
                class="max-w-inherit to-base-100/97 relative size-full bg-gradient-to-b from-base-100/40 from-0% via-base-100/70 via-15% to-30% pt-2">
                ...
                <div
                    ...
                    <div
                        v-if="set"
                        class="col-start-1 grid h-fit w-full grid-flow-row auto-rows-auto grid-cols-3 items-center justify-items-center gap-x-2.5 gap-y-2 p-1.5">
                    ...
                    <div
                        class="relative col-start-2 row-start-2 flex h-full flex-col items-start justify-between gap-4 px-0 py-2 [&_:nth-child(2)]:absolute [&_:nth-child(2)]:right-2 [&_:nth-child(2)]:top-10">
                        <div
