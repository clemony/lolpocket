...
<Accordion default-value="Q" collapsible class='  overflow-hidden  '>
    <AccordionItem v-for="(ability, key) in champion.abilities" :key="key" :value="key" v-model:modelValue="modelValue"
        class='last:border-b-transparent max-h-inherit flex-1 overflow-hidden flex flex-col'>
        <AccordionTrigger class='flex py-2' :class="{ 'h-0': modelValue && !modelValue }">
