...
import PlaceholderSquare from '@components/placeholders/placeholder-square.vue';
...
function makeBox(length) {
    if (length < 6) {
        const length = props.pocket.items[0].start[0].items.length
            ...
            <PopoverContent
                ...
                <Transition name="ah" appear>
                    ...
                    <InfoCard title=" Starter Items" :key="props.pocket.key"
                        ...
                        cardClass="relative overflow-hidden w-full !h-full" noPadding :style="{
                        ...
                        <div class='flex flex-col max-h-inherit overflow-y-auto  mt-2 gap-5 px-7'>
                            <ItemsetModel :pocket="props.pocket" :model="props.model"
                                class='rounded-box bg-base-200 p-2 -ml-2 shadow-sm'>
                                <input type="checkbox" :checked="true"
                                    class='pointer-events-none checkbox checkbox-md place-self-center' />
                            </ItemsetModel>


                            <TransitionGroup name="pop">
                                <label v-for="set in filter" v-close-popper
                                    class="sets flex gap-4 items-center  overflow-hidden w-fit rounded-lg "
                                    :key="set.key">
                                    <template v-if="set.items.length">
                                        <template v-for="(item, index) in set.items" :key="item.id">
                                            ...
                                            <PlaceholderSquare v-for="index in makeBox(set.items.length)"
                                                class='size-[55px]' />
                                        ...
                                        <input type="checkbox" :checked="set == model"
                                            class='pointer-events-none checkbox checkbox-lg' />
                                    </template>
                                </label>
                            </TransitionGroup>
                        </div>
