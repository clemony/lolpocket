        ...
        <ContextMenuTrigger v-if="pocket" class="grid justify-start grid-rows-2 overflow-hidden size-full" @click.stop>
            <div class="relative flex items-end pt-2 group/name">
                <input v-model="pocket.name"
                    class="align-baseline capitalize text-[13px] h-6 truncate transition-all duration-300 bg-transparent px-[4px] font-medium border border-transparent focus:border hover:border focus:border-base-300/50 hover:border-base-300/50 outline-none ring-0 peer py-1 rounded-xs w-full hover:!cursor-text z-0"
                    @keydown.enter.prevent="
                        gs.loseFocus;
                    doThis()
                        " @change="doThis()" spellcheck="false" />
