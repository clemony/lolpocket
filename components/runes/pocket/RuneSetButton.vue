...
<template>
  <button
    v-if="set"
    :key="set.key"
    class="relative !pointer-events-auto size-18 shadow-outline shadow-black/15 grid aspect-square cursor-pointer   place-items-center overflow-visible items-center rounded-full transition-all duration-500 ring-1 ring-b2 "
    :class="{ ' ': set == selectedSet, '': set != selectedSet }"
    @click="handleSet()"
  >
    <div v-if="set.key == pocket.runes.default" v-tippy="'Default Set'" class="absolute size-fit rounded-full shadow-warm-2 drop-shadow-sm -bottom-1 z-1 -left-1.5">
      <input type="checkbox" :checked="set.key == pocket.runes.default" class="peer checkbox checkbox-neutral checkbox-sm drop-shadow-sm rounded-full pointer-events-none " />
    </div>
    ...
    <div
      class="absolute -right-2 -bottom-2  shadow-outline shadow-black/15 grid aspect-square size-11 place-items-center  rounded-full  transition-all duration-500 bg-gradient-to-br from-b2 to-b3 ring-1 ring-b2"
    >
