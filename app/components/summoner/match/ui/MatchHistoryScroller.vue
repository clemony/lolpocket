...
const emit = defineEmits(['update:scroll-y-position'])/*
const rightRef = useDomRef()
const { scrollY } = useScroll({ container: rightRef })
 ...
 */
const matches = computed(() => summoner.matches.full)
const ms = useMatchStore()

const { filteredMatches } = useFilteredMatches(puuid, ms.mf, matches)
    ...
    <DynamicScroller
      ...
      <template #after>
        <div class="h-150 w-220 grid justify-center py-14">
          <GetOldMatchesButton :summoner :loading="loading" />
        </div>
      </template>
    </DynamicScroller>
