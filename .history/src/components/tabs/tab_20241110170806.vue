...
<template><label role="tab" :class="{ 'tab-active': model === props.value }"
    class="tab !z-20 text-base absolute max-w-24 first:left-24 first:order-2  top-0 left-0  [--tab-border-color:oklch(var(--b2))]"
    v-bind="$attrs">
    <input type="radio" class="peer hidden" :value="props.value" :v-model="model" @change="" />
    <slot name="header" />
</label>

<div v-if="props.value === model" role="tabpanel" :class="{ 'rounded-tl-none !max-w-full': model === props.first }"
    class="shadow-sm mb-1 absolute top-8  left-0 flex tab-content !w-inherit min-w-[298px] rounded-box border-base-200 bg-base-100/90 py-4 px-5">
    <slot name="content" />
</div>
</template>
