...
<template>
    ...
    <main
        ...
        class="wrapper size-full overflow-hidden rounded-xl"
        :key="state"
        :class="{
            '': state == 'open',
        }">
        <div
            data-tauri-drag-region
            v-if="state == 'open'"
            class="command margin-auto z-[100] mt-24 overflow-hidden rounded-xl delay-100 animate-in animate-out fade-in-100 slide-in-from-bottom-2/3">
            <Command />
        </div>
        <label class="overflow-hidden rounded-xl">
            <input
                type="checkbox"
            ...
            <SidebarProvider
                class="page overflow-hidden rounded-2xl bg-base-100/95 animate-in zoom-in-[0.75] slide-in-from-bottom-1/3"
                :class="{
                    'absolute shadow-[rgba(0,_0,_0,_0.2)_0px_30px_90px,_2px_1px_1px_2px_rgba(221,_221,_221,_0.4),_0_2px_5px_rgba(204,_204,_204,_0.4)] animate-out zoom-out-[0.75] slide-out-to-bottom-1/3 after:absolute after:z-50 after:size-full after:bg-black/20':
                        state == 'open',
                }">
                <MainMenubar
                    :class="{ '!z-0': state == 'open' }"
                    class="menubar" />
