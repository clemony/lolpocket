...
const maskSize = computed(() =>
  typeof props.maskSize === "number"
    ? `var(--spacing) * ${props.maskSize}`
    : props.maskSize
)
...
<template>
  <div
    ...
    <div
      ...
      ">
      <div
        class="grid h-full overflow-hidden transition-transform delay-(--delay) duration-(--duration) ease-(--easing) will-change-transform group-hover/hover-card:filter-none group-hover/hover-card:[--duration:200ms] group-hover/hover-card:[--easing:linear] group-hover/hover-card:[--opacity:0.6]">
        <div
          :class="
            cn(
