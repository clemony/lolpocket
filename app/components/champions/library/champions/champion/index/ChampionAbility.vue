...
<template>
  <Collapsible
    :id="ability.key"
    :default-open="true"
    class="flex field-box  max-w-220 flex-col  **:text-3 gap-6 w-full justify-center">
    <CollapsibleTrigger class="px-6 py-4 data-[state=open]:border-b border-b-b3  flex items-center justify-items-start gap-6">
      <div
        class="size-18 rounded-lg shadow-sm drop-shadow-sm overflow-hidden">
        <Img
          :img="ability.icon"
          :alt="ability.name" />
      </div>
      ...
      <div class="gap-2 flex flex-col grow justify-center ">
        ...
        <div
          v-if="ability && (ability.resource || ability.cooldown?.length || ability.rechargeRate || ability.cost?.length || ability.effectRadius || ability.targetRange)"
