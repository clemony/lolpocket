...
<template><label role="tab" :class="{ 'tab-active': tabValue === props.value }"
    class="tab !z-20 text-base !max-w-full [--tab-border-color:oklch(var(--b2))]" @click="updateTab(props.value)"
    v-bind="$attrs">
    <slot name="header" />
</label>

<div v-if="tabValue === props.value" role="tabpanel" :class="{ 'rounded-tl-none !max-w-full': props.value === 1 }"
    class="shadow-sm mb-1 absolute top-8 pr-px left-0 flex tab-content !w-inherit min-w-[298px] rounded-box border-base-200 bg-base-100/90 py-4 px-5">
    <slot name="content" />
</div>
</template>
