...
<div class='flex flex-col max-h-inherit gap-10 w-full' ref="card">
    ...
    <InfoCard title="Starter Items"
        ...
        cardClass="max-h-inherit relative  pl-6 pt-3  pb-5 pr-14 has-[aria-expanded=true]:invisible has-[aria-expanded=true]:opacity-0 transition-all duration-500"
        ...
        <Dialog>
            <Popover>
                ...
                <DdItemset @update:model="(model) => pocket.items[0].start[0] = model" v-model:model="start"
    ...
    <InfoCard title="Core Items" description="Your champion's core. What should you work towards first? "
        cardClass="max-h-inherit relative w-fit pl-6 pt-3  pb-5 pr-14 has-[aria-expanded=true]:invisible has-[aria-expanded=true]:opacity-0 transition-all duration-500"
        v-model:cardWidth="cardWidth" v-model:cardHeight="cardHeight">
        <Dialog>
            <Popover>
                <DialogTrigger>
                ...
                <DdItemset @update:model="(model) => pocket.items[0].core[0] = model" v-model:model="core"
                    :sets="pocket.items[0].itemSets" :cardWidth="cardWidth" :cardHeight="cardHeight" :pocket="pocket" />
    ...
    <InfoCard title="Final Build"
        ...
        <Dialog>
            <Popover>
                ...
                <DdItemset @update:model="(model) => pocket.items[0].final[0] = model" v-model:model="final"
                    :sets="pocket.items[0].itemSets" :cardWidth="cardWidth" :cardHeight="cardHeight" :pocket="pocket" />
            </Popover>
