...
<VueDraggable ref="el" :group="{
    ...
    class="drag-draggable rounded-lg items-start justify-around h-inherit max-h-inherit pr-10 pb-4 pt-4 overflow-y-auto scrollbar-hide">
        ...
        <div v-for="(champion, index) in filteredChampions" :key="champion.name" ref="list" class="size-[68px]">
            ...
            <label class="drag-label" dragClass="setDrag" @click.right.prevent="">
                <div class="drag-wrapper">
                    ...
                    <LoadImg :url="`/img/champions/${clean(champion.name)}.webp`" class="load-img drag-img scale-[108%]"
                        draggable="false" />
                </div>
            </label>
