...
<template>
    <ScrollArea as-child>
        <div
            class="scroll mb-10 flex h-full max-h-64 w-full flex-row flex-wrap justify-evenly gap-2 overflow-y-scroll p-1">
            <Label
                variant="ghost"
                v-for="icon in iconStore"
                class="aspect-square size-16 self-center rounded-md border-transparent p-4.5 has-[:checked]:bg-base-200/60 has-[:checked]:shadow-standard">
                <input
                    v-if="pocket"
                    type="radio"
                    name="iconPicker"
