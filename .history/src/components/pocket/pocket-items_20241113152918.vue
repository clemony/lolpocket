...
console.log('el value logged in parent', el)
...
onMounted(() => {
    if (el.value) {
        // Ensure the scroll element reference is set up
        console.log('Scroll element is set:', el.value)
    }
})
// Watch the top state, which is now wrapped in a ref
watch(toRef(arrivedState, 'top'), (newVal) => {
    console.log('Top arrived:', newVal)
    isTop.value = newVal // Update `isTop` which is now mutable
})
</script>
