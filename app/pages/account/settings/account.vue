...
<template>
  <form v-if="user" class="w-full space-y-12" @submit="onSubmit">
    <!-- username -->
    ...
    <fieldset id="region" class="space-y-6">
      <div class="leading-4">
        <h4 as="legend" class="mb-2 text-5 font-semibold">Region</h4>

        <p class="label text-wrap">Your default summoner search region.</p>
      </div>
      ...
      <ClientOnly>
        ...
        >
          <SelectTrigger class="input flex h-12 w-full inset-shadow-none">
            <!--       <span class="grow">
              {{ select }}
            </span> -->
            <SelectValue />
    ...
    <fieldset id="connected-accounts" class="space-y-2">
      <div class="leading-4">
        <h4 as="legend" class="mb-2">Connected Accounts</h4>
      ...
      >
        <div
          ...
          "
        >
          <icon :name="String(provider.icon)" :class="cn('size-6.5 dst')" />
        </div>
        <h4 class="grow text-start text-4 font-semibold capitalize dst">
          {{ provider.name }}
        </h4>

        <span v-if="userProviders?.includes(provider.name)" class="text-2">
          Connected
        </span>

        <span v-else class="text-2 opacity-60">Not Connected</span>
    ...
    <fieldset id="connected-accounts" class="space-y-6">
      <div class="leading-4">
        <h4
          as="legend"
        ...
        <p class="label text-wrap">Used for password reset and update.</p>
      </div>
      ...
      >
        ...
        <template #2>
          <Button
            ...
            size="6"
          >
            <icon name="tick" class="absolute size-4.25 **:stroke-[3.3]" />
          </Button>
          <Button
            v-else
            v-tippy="{
    ...
    <fieldset id="blocked-users" class="space-y-6">
      <div class="leading-4">
        <h4 as="legend" class="mb-2 text-5 font-semibold">Blocked Users</h4>
