...
<ContextMenu class="context-menu">
    <ContextMenuTrigger
        class="group  size-full cursor-pointer items-center justify-start content-center relative flex px-2 gap-2"
        v-if="pocket">
        <template v-for="(champion, index) in pocket.champions[0].champions">
            <div v-if="index < 3" class='overflow-hidden size-14 rounded-lg shadow-sm border border-base-300' v-cosha>
