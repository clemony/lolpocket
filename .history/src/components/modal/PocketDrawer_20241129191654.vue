...
<template>
    <div class="relative w-full justify-center py-2">
        <form
            class="flex gap-12 py-4 [&_label]:text-base"
            @submit.prevent.stop="submitForm">
            <div ref="elForm" class="ml-px flex w-1/2 flex-col gap-2 pl-4">
            ...
            <DialogFooter class="absolute bottom-1 left-0 w-full px-12 py-5">
                <DialogClose>
                    <Button
                        @click="submitForm"
                        v-if="!gs.modalPocket"
                        variant="neutral"
                        type="submit"
                        class="btn btn-sm flex cursor-pointer items-center justify-self-end">
                        {{ submitText }}
                    </Button>
