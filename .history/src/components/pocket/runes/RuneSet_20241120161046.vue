...
<template v-if="set">
    <template v-for="rune in this1">
        <div
            class="absolute shrink-0 rounded-full border border-base-300/70 bg-base-100/50 p-1.5 shadow-[0_3px_10px_rgb(0,0,0,0.2),inset_-1px_-1px_1px_0px_#00000008] backdrop-blur-sm">
            <template v-if="rune.name != 'none'">
                <LoadImg
                    :url="rune.img"
                    class="size-[2.8rem] rounded-full shadow-sm drop-shadow-sm" />
            </template>
            <Placeholder
                v-else
                :size="35"
