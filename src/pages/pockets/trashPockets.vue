...
<template>
  ...
  <div class="grid w-full px-3 py-4">
    ...
    <div
      class="grid  grid-cols-[30px_1.5fr_2fr_3fr_1fr_repeat(2,30px)]  gap-x-2  *:text-xs items-center mt-1 mx-3 py-1 border-b border-b-base-300 *:flex *:gap-1 [&_svg]:mt-0.5">
      <!-- head -->
    ...
    <div class="grid h-full grid-cols-1 py-2 border-b border-b-base-300 ">
      ...
      <div v-for="pocket in ps.pockets" :key="pocket.key"
        ...
        <div class="grid grid-cols-[40px_30px_auto] gap-3 items-center">
          ...
          <template v-if="pocket.runes?.[0]?.starred?.[0]?.secondary?.length > 0">
            <img :src="`/img/runes/${pocket.runes[0].starred[0].secondary}.webp`"
              class="w-auto h-6 drop-shadow-softest" />
          </template>
        </div>
        ...
        <div class="h-full flex items-center content-center !self-center w-12  gap-2" @click.stop>
          <input type="checkbox" :value="pocket.key" class="checkbox checkbox-xs" @click.stop
            v-model="selectedPockets" />
