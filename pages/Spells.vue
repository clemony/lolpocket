...
const spellCompute = computed (() => {
  const sp = ref(summonerSpells[selectedSpell.value])
    ...
    <div class="w-full  flex gap-[8%] px-3 py-24">
      <div class="flex flex-col gap-14 w-114">
        <div class="grid grid-cols-3 grid-rows-3 gap-4 py-8 border-b3/80 rounded-box shadow-smooth  px-9 h-fit  ">
          <div v-for="(spell, i) in summonerSpells" :key="spell.name" v-tippy="spell.name" class="size-fit">
        ...
        <div :key="selectedSpell" class="flex flex-col gap-14 w-160 transition-all duration-300">
          <div class="border-b3/80 rounded-xl shadow-smooth pt-7 pb-8 px-9  w-full border flex max-h-min flex-col gap-6 transition-all duration-300">
            <div class="w-full flex gap-6 i-c">
              <img :src="`/img/spells/${summonerSpells[selectedSpell].name.toLowerCase()}.webp`" class="rounded-lg shadow-sm  shadow-black/20 size-24" />
              <div class="h-full grid items-between py-2">
                <p class="font-medium">
                  Summoner
                </p>
                <h1 class="text-9 dst tracking-tight">
                  {{ summonerSpells[selectedSpell].name }}
                </h1>
              </div>
            </div>

            <p>{{ summonerSpells[selectedSpell].description }}</p>

            <ul class="w-74 space-y-3">
              <li v-if="summonerSpells[selectedSpell].cd" class="grid-cols-2 grid gap-3  items-end ">
                <p class="font-semibold tracking-tight">
                  Cooldown:
                </p>
                <p class="font-medium">
                  {{ summonerSpells[selectedSpell].cd }} seconds
                </p>
              </li>

              <li v-if="summonerSpells[selectedSpell].charges" class="grid-cols-2 grid gap-3  items-end ">
                <p class="font-semibold tracking-tight">
                  Charges:
                </p>
                <p class="font-medium">
                  {{ summonerSpells[selectedSpell].charges }}
                </p>
              </li>

              <li v-if="summonerSpells[selectedSpell].recharge" class="grid-cols-2 grid gap-3  items-end ">
                <p class="font-semibold tracking-tight">
                  Recharge Time:
                </p>
                <p class="font-medium">
                  {{ summonerSpells[selectedSpell].recharge }} seconds
                </p>
              </li>
            </ul>
          </div>
