...
<VueDraggable v-if="pocket" tag="div" v-model="pocket.items[0].itemSets" :delay="0" :animation="300"
    ...
    <InfoCard v-for="(set, index) in pocket.items[0].itemSets" dragClass="setDrag" :key="set.key" headerClass="pb-2"
        ...
        <template #header>
            ...
            <div class='w-fit -mt-1 -ml-2'>
                <InputEditable v-model:modelValue="set.name" :defaultValue="set.name" @click.stop
                    class='!text-base font-medium text-start items-center align-baseline capitalize  border-transparent focus-within:border-base-200 hover:border-base-200 transition-all duration-300'>
                    <button>
                        <icon icon="teenyicons:x-outline" class='p-[2px] -mt-[3px]' />
                    </button>
                    <button @click.stop="
                        set.name = generateRandomName() + ' Set'">
                        <icon icon="bitcoin-icons:question-filled" class='size-[16px] opacity-50' />
                    </button>
                </InputEditable>
            </div>
