    ...
    <AccordionItem v-for="(ability, key) in champion.abilities" :key="key" :value="key"
        ...
        <AccordionTrigger class='flex py-2'>
            ...
            <div :style="{ 'backgroundImage': 'url(' + ability.img + ')' }" :alt="ability.name"
                class="rounded-lg shadow-sm border border-base-300 size-[45px] colorful-shadow bg-contain"></div>
...
.colorfulShadow::after {
    ...
    filter: drop-shadow(0px 0px 10px rgba(0, 0, 0, 0.50)) blur(10px) saturate(150%);
    z-index: -1;
}
</style>