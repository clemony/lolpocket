...
const runeSet = pocket.runes[0].runeSets[pocket.runes[0].starred];

const keystone = computed(() => {
    const stone = runeSet.keystone;
    if (runeSet && keystone) {
        return stone;
    }
});
...
console.log('hi', pocket.runes[0].runeSets)
...
<template>
    ...
    <ContextMenu class='context-menu'>
        <ContextMenuTrigger class="relative flex items-start justify-start gap-4 size-full opacity-95 group">
            <div class='flex gap-3 size-full'>
                <div v-if="keystone && keystone.name != 'none'"
                    class="relative flex items-start justify-start h-full overflow-hidden rounded-md shadow-sm cursor-pointer aspect-square bg-gradient-to-br from-transparent to-75%"
                    :class="{
                    ...
                    <div
                        class="flex items-start justify-start gap-2 p-2 rounded-md size-full bg-gradient-to-br from-base-100/90 to-base-100/40 backdrop-blur-md">
                        <img :src="keystone.img"
                            class="!object-contain drop-shadow-softer size-10 hover:drop-shadow-sm transition-all duration-200  hover:opacity-80" />


                    </div>

                    <div v-if="runeSet.secondary && runeSet.secondary != 'none'" :class="{
                        ...
                        class="rounded-full absolute right-[1px] !aspect-square cursor-pointer bg-base-100/95 backdrop-blur-md overflow-clip !size-6 bottom-[1px]  border-2 p-[2px] shadow-sm">
                ...
                <div v-else @click.stop="sn.navigateTo(`/pocket/${pocket.key}/pocket-runes`, pocket.name, pocket.icon)"
                    class="flex group items-center rounded-[4px] justify-center !justify-self-start  w-auto h-full gap-2 border shadow-sm opacity-70 aspect-square bg-base-200/30 border-base-200 cursor-pointer hover:opacity-70 transition-all duration-300 hover:border-base-300 hover:shadow-inner ">
            ...
            <TableContextMenu type="runes" :params="props.params">
                <ContextMenuSub>
                    <ContextMenuSubTrigger :disabled="pocket.runes[0].runeSets <= 1"
                        :class="{ 'last:[&_svg]:hidden': pocket.runes[0].runeSets <= 1, 'opacity-50': pocket.runes[0].runeSets == 0 }">
                        ...
                        <ContextMenuItem v-for="(set, index) in pocket.runes[0].runeSets">
                            ...
                            <label class="flex items-center gap-0.5">
                                ...
                                <div class="flex [&_img]:size-5">
                                    ...
                                    <img v-if="set.s3" :src="set.s3.img" />
                                </div>
