...
const { id, class: className, runeSize = 16 } = defineProps<{
  id: number
...
const imgSize = computed (() => rune.value.tier === 0 ? `size-${runeSize + 1} ` : `size-${runeSize}`)
watchEffect(async () => {
  if (!id)
    return
...
<template>
  <div
    v-if="rune"
    :key="rune.id"
    :class="cn('animate-in fade-in tldr-30 relative flex  w-full flex-col justify-center py-3', className)">
    <div
      class="flex w-full items-center  gap-4  px-4    **:select-none  ">
      <button
        v-tippy="{ content: rune.tier === 0 ? 'Keystone' : null, theme: 'neutral', placement: 'top-start', offset: [-12, 8] }"
        :class="cn('group/s shrink-0 relative', wrapperSize)">
        <span :class="cn('absolute top-0 left-0', imgSize)">
          <Img
            ...
            :alt="`${rune.name} Image`"
            :class="cn('pointer-events-none size-full absolute object-contain  rounded-full  inset-shadow-sm top-0 left-0 shadow-sm drop-shadow-sm')" />
        </span>
        <icon
          v-if="rune.tier === 0"
          name="star-fill"
      ...
      <div class="flex h-full w-full  flex-col justify-center gap-1">
        <div class="flex items-center justify-between">
          <h2
            class=" dst grow    leading-none">
            {{ rune.name }}
          </h2>
          <a
            :key="rune.id"
            v-tippy="{ content: `Official LoL Wiki - ${rune.name}`, theme: 'neutral', placement: 'top-end', offset: [12, 8] }"
        ...
        <div class="flex w-full items-center gap-3 ">
          ...
          <span class="text-4 grow font-medium italic">{{ rune.path }}</span>
        </div>
      </div>
    </div>
