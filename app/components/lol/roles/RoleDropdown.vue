...
<template>
  <DropdownMenu>
    <DropdownMenuTrigger as-child>
      ...
      >
        <component
          ...
          class="drop-shadow-text text-md! size-6.5 shrink-0 text-white/70 focus:outline-0"
        />
    ...
    <DropdownMenuContent key="role" class="w-48" side="bottom">
      <DropdownMenuItem>
        <label class="hover:bg-b2/60! flex gap-4 text-sm" for="all">
          <input
            id="all"
      ...
      <DropdownMenuItem v-for="role in roles" :key="role" class="">
        <label class="hover:bg-b2/60! flex gap-4 text-sm" :for="role">
          <input
            :id="role"
