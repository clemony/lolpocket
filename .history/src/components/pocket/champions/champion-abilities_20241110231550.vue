...
<Accordion default-value="Q" collapsible class='  overflow-hidden' v-model:modelValue="modelValue"
    ...
    <AccordionItem v-for="(ability, key) in champion.abilities" :key="key" :value="key"
        :class="{ 'h-0': modelValue && modelValue != key, ' border-transparent': modelValue }"
        class='last:border-b-transparent max-h-inherit flex-1 overflow-hidden flex flex-col transition-all duration-500'>
