...
<template>
    <template v-for="(item, index) in items" :key="item.id">
        <div
            v-if="index < 6"
            :key="item.id"
            class="shrink-0 rounded-lg border border-base-300/70 bg-base-100/50 p-0 shadow-warm backdrop-blur-sm">
            <Item
                :item="item"
                :pocket="pocket"
                imgClass="rounded-lg shadow-sm drop-shadow-sm !p-0 m-0 object-center  size-full border border-base-300"
                class="!size-14 !p-0 ring-0 first:!rounded-tl-full hover:ring-0"
