    ...
    <div class="flex flex-wrap gap-x-2.5 gap-y-2">
        <template v-for="(item, index) in items" :key="item.id">
            <div
                v-if="index < 6"
                :key="item.id"
                class="size-11 shrink-0 rounded-lg bg-base-100/50 p-0 shadow-warm backdrop-blur-sm [&_first-child]:!rounded-tl-full">
                <Item
                    :item="item"
                    :pocket="pocket"
                    imgClass="rounded-lg shadow-sm drop-shadow-sm !p-0 m-0 object-center  size-full border border-neutral/60"
