...
<template><!--     <div v-if="is.sortAZ != '' || is.sortPrice != ''" class="z-10 flex items-center gap-3 justify-self-start btn-xs">
      <div
      ...
      <Icon icon="iconoir:bin-full" class="self-center  size-[1rem] p-0" @click="is.sortAZ = ''; is.sortPrice = ''"
...
<div class="grid">
    ...
    <div class="-ml-2 grid w-full items-center justify-center gap-3 pb-3 pt-1">
        <div class="relative flex w-full items-center gap-2 px-1">
            <label :class="{ 'opacity-40': is.sortAZ != 'az' }">
                <input type="radio" value="az" v-model="is.sortAZ" class="peer hidden" />
                <kbd class="kbd aspect-square overflow-clip p-0 *:size-6">
                    <icon icon="mynaui:letter-a" />
                </kbd>
            </label>

            <button v-if="is.sortAZ == 'az'" @click="is.sortAZ = ''"
                class="absolute left-[5px] top-0.5 z-10 size-6 opacity-0"></button>

            <icon v-if="is.sortAZ == ''" icon="codicon:arrow-swap" class="mb-1" />

            <icon v-if="is.sortAZ != ''" :class="{ 'rotate-180': is.sortAZ == 'za' }" icon="codicon:arrow-right"
                class="mb-1 transition-all duration-300" />

            <label :class="{ 'opacity-40': is.sortAZ != 'za' }">
                <input type="radio" value="za" v-model="is.sortAZ" class="peer hidden" />
                <kbd class="kbd aspect-square overflow-clip p-0 *:size-6">
                    <icon icon="mynaui:letter-z" />
                </kbd>
            </label>

            <button v-if="is.sortAZ == 'za'" @click="is.sortAZ = ''"
