...
function handleChampions(champion: string) {
  if (pocket.value.champions.includes(champion))
    return
...
const pagedItems = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage
  return pocket.value.champions.slice(start, start + itemsPerPage)
})
  ...
  <Popover v-model:open="open">
    <PopoverTrigger
      ...
      <Button
        ...
        <div class="size-full opacity-0 group-hover/collapse:opacity-100 group-data-[state=open]/collapse:opacity-100 transition-opacity duration-200 bg-neutral/70 absolute grid place-items-center">
          <CaretFlip
