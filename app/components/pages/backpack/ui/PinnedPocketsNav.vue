...
<template>
  ...
  >
    <CollapsibleTrigger as-child class="group/collapse">
      <Button as="button" variant="ghost" class="w-full" size="md">
        <span
          class="grow text-4 font-semibold tracking-normal capitalize opacity-40"
        >
          Pinned Pockets
        </span>
        <CaretRotate />
      ...
      >
        ...
        >
          <BtnLink
            ...
            :to="`/${link.key}`"
            class="flex grow items-center justify-start gap-3! px-2 text-3 duration-0!"
          >
            <PocketIcon
              size="sm"
              :img="link.icon"
          ...
          <Label
            ...
            @click.stop
          >
            <icon name="pin" class="absolute size-4 **:stroke-[1.84]" />
            <input
              v-model="ps().pinned"
              name="toggle-pin"
              type="checkbox"
  ...
  <nav v-else class="flex flex-col items-center justify-center gap-1">
    <BtnLink
      ...
      variant="ghost"
    >
      <PocketIcon size="sm" :img="link.icon" class="size-9 rounded-full" />
    </BtnLink>
  </nav>
</template>
