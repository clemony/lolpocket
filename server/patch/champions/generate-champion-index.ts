...
const dataPath = resolvePath("./champions/raw/champions-raw-data-dragon.json")
...
for (const champ in champs) {
  ...
  index.push({
    id: Number.parseInt(champion.key), // data dragon is reversed (dd.key === my.id)
    key: champion.id, // reversed
    name: champion.name,
  })
}
...
/* for (const key in champs) {
  ...
  if (champion.key && champion.positions[0])
    roleIndex[champion.key] = champion.positions[0]
} */
...
export const championRoleIndex: Record<string, string> = ${JSON.stringify(roleIndex, null, 2)}
` */
