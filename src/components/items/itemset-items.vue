...
<VueDraggable ref="el" v-model="set.items" :group="{
    ...
    filter=".no-drag" :force-fallback="true" :fallbackOnBody="true" @end="handleDragEnd"
    class="drag-draggable item !justify-start overflow-hidden !p-1 scrollbar-hide" :class="{ '!gap-5': props.limit }">
    <template v-for="(item, index) in set.items" :key="item.id">
        <VDropdown v-if="!props.limit || (props.limit && index < 6)" :overflow-padding="20" :shift="true" theme="detail"
            :distance="6" @click.right.prevent="" :ref="item.name" class="relative max-h-[64px] max-w-[64px]">
