...
const filteredChampions = computed(() => {
    ...
    if (cs.champSearch) {
        filtered = filtered.filter((champion) => Object.values(champion).some((value) => typeof value === 'string' && value.toLowerCase().includes(cs.champSearch)));
    }
...
<template>
    <VueDraggable ref="el" :group="{ name: 'champs', pull: 'clone', put: false, revertClone: true }" :sort="false"
        v-model="filteredChampions" ghostClass="ghosty" :delay="0" :animation="300" :force-fallback="true"
            ...
            <div v-for="(champion, index) in filteredChampions" :key="champion.name" ref="list" class=" basis-18">
                <label class="drag-label " dragClass="setDrag" @click.right.prevent="">
                    ...
                    <div class="drag-wrapper">
                        ...
                        <img :src="`/img/champions/${champion.name.replace(/\s/g, '').replace(/\'/g, '').replace(/\./g, '').replace(/\&/g, '')}.webp`"
                            class="load-img scale-[115%] drag-img" draggable='false' />
                    </div>
