...
const headerVariants = {
  ...
  transition: {
    delay: 0.2,
    duration: 0.8,
    ease: [0.215, 0.610, 0.355, 1.000] as unknown as Easing[],
  },
}
...
const titleVariants = {
  ...
  exit: {
    ...
    transition: {
      duration: 0.8,
      ease: [0.550, 0.085, 0.680, 0.530] as unknown as Easing[],
    },
  },
}
...
<template>
  <div class=" max-w-400 w-full px-10 flex flex-col justify-center gap-4">
    <div
      class="h-18 relative pl-1 pr-2 w-full">
      <div
        v-if="pathHovered"
        ...
        <h1
          v-if="pathHovered"
          :key="pathHovered"
          :class="cn('dst duration-600 ', { 'animate-in fade-in blur-in-50 zoom-in-120': pathHovered, 'animate-out fade-out blur-out-50 zoom-out-120': !pathHovered })">
          {{ pathHovered }}
        </h1>
        <p
          v-if="pathHovered"
          :key="pathHovered"
          :class="cn('dst font-medium duration-600  pt-1 pr-2 text-end', { 'animate-in fade-in ': pathHovered, 'animate-out fade-out': !pathHovered })">
