...
<template>
  <div class=" w-full min-h-24 overflow-hidden bg-b2 field-box border-b3/60 p-3 py-3 px-2" @click="navigateTo(`/pocket/${pocket.key}`)">
    <AnimatePresence as="div" layout mode="sync" class="size-full relative min-h-24 gap-2 justify-evenly items-center flex-wrap flex">
      <button v-if="!pocket?.champions || !pocket?.champions?.length" class="size-full cursor-pointer grid place-items-center absolute group/box">
