...
<template><label role="tab" :class="{ 'tab-active': model === props.value }"
    class="tab !z-20 text-base absolute max-w-24   top-0 left-0  [--tab-border-color:oklch(var(--b2))]" v-bind="$attrs">
    <input type="radio" class="peer hidden" :value="props.value" :v-model="model"
        @change="emit('update:modelName', model)" />
    <slot name="header" />
</label>

<div v-if="props.value === model" role="tabpanel" :class="cn('', { 'rounded-tl-none': props.first })"
