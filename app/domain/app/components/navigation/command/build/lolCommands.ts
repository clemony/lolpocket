...
export const referenceItems = computed(() => {
  ...
  const items: CommandRouteItem[] = uniqueItems.map((entry) => ({
    label: entry.name,
  ...
  const champions: CommandRouteItem[] = championIndex
    .filter((entry) => entry?.id != null && !!entry.name)
    .map((entry) => ({
      label: entry.name,
      id: entry.id.toString(),
  ...
  const runes: CommandRouteItem[] = runeIndex
    .filter((entry) => entry?.id != null && !!entry.name)
    .map((entry) => ({
      label: entry.name,
      id: entry.id.toString(),
  ...
  const spells: CommandRouteItem[] = Object.values(spellIndex)
    .filter((entry) => entry?.id != null && !!entry.name)
    .map((entry) => ({
      label: entry.name,
      id: entry.id.toString(),
