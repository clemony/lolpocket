...
<VDropdown v-if="data" theme="select" placement="bottom" class="relative flex self-end p-0 justify-center"
    :skidding="1">
    <Button variant="outline" size="xs" class="text-start pointer-events-auto w-36 justify-start">

        {{ dataValue ? dataValue : props.defaultText }}
    ...
    <template #popper>
        <div class='flex w-36 max-w-36 flex-col p-1 pb-8 overflow-hidden'>
            ...
            <label v-for="data in props.data">
                <Button variant="ghost" size="xs" class='w-full  !justify-start text-start'>


                    <input type="checkbox" v-model="dataValue" :value="data" class="peer hidden" />
                    <span class='grow'>{{ data }}</span>

                    <icon v-if="dataValue == data" icon="teenyicons:tick-outline" class="size-3" />
