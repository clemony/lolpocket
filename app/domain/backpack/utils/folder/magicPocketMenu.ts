import { SparkleIcon } from "#components"
import type { ContextMenuItem } from "@nuxt/ui"
import { separatorItem } from "~/domain/backpack/utils/folder/menuItems"
import { newRandomPocket } from "~/domain/pocket/utils/newRandomPocket"
import { itemLabelUi } from "~~/layers/ui/app/variants/menu"

interface NewOptions extends ContextMenuItem {
  location: string | undefined
  folder?: {
    disabled: boolean
  }
  pocket?: {
    disabled: boolean
  }
}
const map = shallowRef<MapKey>(11)
const position = shallowRef<PositionKey>("all")
const mapOpen = shallowRef(false)
const positionOpen = shallowRef(false)

const indicator: Record<string, string> = {
  size: "sm",
  variant: "ghost",
  color: "base"
}

const labelUi = {
  ...itemLabelUi,
  label: cn("pb-2!", itemLabelUi.label)
}

const maps = {
  SR: 11,
  ARAM: 12,
  All: 0
}

const mapItems = Object.entries(maps).map(([label, value]) => ({
  label,
  icon: `i-lp-${value}`,
  value: value as MapKey,
  ui: {
    item: "col-start-2 gap-2",
    itemLabel: "capitalize font-medium",
    itemLeadingIcon: " scale-110"
  }
}))

const positionItems = positionSchema.options.map((p) => ({
  label: p,
  icon: `i-lp-${p}`,
  value: p,
  ui: {
    itemLabel: "capitalize",
    item: "gap-2 checked:bg-p2/60",
    itemTrailing: "rounded-full",
    itemLeadingIcon: " scale-110"
  }
}))

const magicChildren = [
  {
    type: "label",
    label: "Generation Defaults"
  },
  {
    type: "separator"
  },
  {
    type: "label",
    label: "Position",
    slot: "select",
    ui: labelUi,
    select: {
      icon: `i-lp-${position.value ?? "all"}`,
      modelValue: position,
      items: positionItems,
      open: positionOpen,
      ui: { content: " bg-p0/94 p-0.5" }
    }
  },
  {
    type: "label",
    label: "Map",
    slot: "select",
    ui: labelUi,
    select: {
      icon: `i-lp-${map.value ?? 0}`,
      modelValue: map,
      items: mapItems,
      open: mapOpen,
      ui: {
        content: " bg-p0/94 p-0.5 max-h-100 h-max!"
      }
    }
  }
]

export const magicPocketMenu = (location: string) =>
  computed(() => [
    {
      icon: h(SparkleIcon, {
        ui: {
          root: "size-4.5",
          icon: "size-4!",
          inactveIcon:
            "group-aria-[expanded=true]/btn:opacity-0 group-data-[state=open]/btn:opacity-0",
          activeIcon:
            "group-aria-[expanded=true]/btn:opacity-100 group-data-[state=open]/btn:opacity-100"
        }
      }),
      label: "Magic Pocket",
      ui: {
        item: "group/btn",
        content: "max-w-64 pb-0.5"
      },

      content: {
        // sideOffset: options?.content?.sideOffset ?? -4,
        align: "start"
      },
      children: magicChildren
    },
    separatorItem,
    {
      label: "Generate",
      color: "neutral",
      icon: "i-activity",
      variant: "solid",
      ui: {
        item: " px-3 group/item gap-2 mb-1 mt-1 flex justify-center rounded-lg bg-neutral hover:bg-n3 fx-noise-3 fx-1 justify-center w-[calc(100%-0.3rem)] mx-auto",
        itemLabel: "text-nc! font-semibold",
        itemLeadingIcon:
          "text-nc scale-110 group-hover/item:text-nc! group-[data=highlighted]/item:text-nc! group-active/item:text-nc! text-nc! **:text-nc!"
      },
      onSelect() {
        newRandomPocket({
          map: map.value,
          location: String(location),
          position: position.value
        })
      }
    }
  ])
