...
<div class="relative grid w-72 !min-w-60 max-w-96 overscroll-none p-4">
    <div class="grid w-full grid-cols-[1fr_2fr] gap-2 border-b border-base-300 pb-3">
        <div class="col-start-1 h-full">
            <img :key="item.id" :src="`/img/items/${item.id}.webp`" :alt="item.name + ' Image'"
                class="shadow-warm pointer-events-none size-14 rounded-md border border-base-300" />
        </div>
        ...
        <div class="grid h-full grid-cols-1">
            ...
            <div class="flex items-end justify-end gap-2">
                ...
                <a :href="item.wiki" target="_blank"
                    class="btn btn-circle btn-neutral btn-xs aspect-square hover:opacity-75" alt="link to league wiki">
 ...
 -->
                ...
                <VDropdown v-if="props.variant == 'add'" theme="default" :key="item.id + 'dropdown'"
                    ...
                    <template #popper :key="item.name + 'addToSet'">
                        <ul class="pointer-events-auto grid !w-full min-w-28 items-center gap-2 p-2">
                            <span class="cursor-default pl-2">Add to Set</span>
                            <li v-for="set in is.itemSets" class="btn btn-ghost btn-xs justify-start" @click="
    ...
    <div :key="item.name + 'statsdiv'" v-show="item.stats || item.active.length > 2 || item.passive.length > 2
        " class="grid grid-cols-1 gap-2 self-start whitespace-pre-line border-b border-base-300 py-2 ">
        <p v-html="item.stats" v-if="item.stats" class="capitalize leading-5"></p>

        <p v-html="item.active" v-if="item.active.length > 2"></p>

        <p v-html="item.passive" v-if="item.passive.length > 2" :key="item.name + 'Passive'" class="text-pretty"></p>
    </div>
    ...
    <div class="flex items-end pt-2">
        ...
        <p v-if="item.buy" class="flex items-center justify-end gap-1 self-end px-1 ">
