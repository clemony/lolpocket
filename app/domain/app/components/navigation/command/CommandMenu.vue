...
} from "../../../../../utils/routes/useRouteGroups"
...
const stack = shallowRef<Array<{ title: string; items: RouteItem[] }>>([])
...
function openItem(item: RouteItem) {
      ...
      <div
        ...
        <div class="w-full px-3">
          <template v-for="(item, i) in nexusToolsGroup" :key="i">
            <RouteGroup v-if="item" :items="item" class="border-y-0" />
          </template>
        </div>
        <ReferenceCommand
          v-if="groupMap?.reference"
          :reference="groupMap?.reference"
    ...
    <template v-else>
      <UButton
        ...
        <div
          ...
          <span
            v-if="(activeLead as CommandRouteItem)?.suffix"
            class="truncate text-xs text-n4">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-3">
        ...
        <div v-else class="space-y-1">
          <CommandButton
            ...
            :item
            :value="item.value"
            @update:open="(item: RouteItem) => openItem(item as RouteItem)" />
        </div>
      </div>
    </template>
  </div>
