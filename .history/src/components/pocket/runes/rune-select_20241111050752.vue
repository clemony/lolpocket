...
<div v-if="props.set == 1" class="my-9 grid grid-cols-3 place-items-center justify-center gap-x-2 gap-y-9 px-7">
    <label v-for="(rune, index) in filter" :key="rune.name"
        ...
        <VDropdown v-if="rune.tier == 0" theme="default" :triggers="['hover']" :popperTriggers="['hover']"
            ...
            class="h-[inherit] w-[inherit] opacity-80 [&_.one]:opacity-0 [&_.2]:opacity-0 transition-all duration-500 hover:opacity-100  peer-checked:opacity-100  relative fade fade-4">
