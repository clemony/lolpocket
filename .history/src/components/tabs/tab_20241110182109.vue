...
<div v-if="props.value === model" role="tabpanel" :class="{ 'rounded-tl-none': props.first }"
    class="shadow-sm mb-1 absolute top-8  left-0 flex tab-content !w-inherit min-w-[298px] rounded-box border-base-200 bg-base-100/90 py-4 px-5 overflow-hidden flex-col flex-1  h-[calc(100%-20px)]">
    <slot name="content" />
</div>
</template>
