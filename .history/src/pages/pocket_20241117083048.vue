...
const Dashboard = defineAsyncComponent(
    () => import('../components/Pocket/PocketDashboard.vue')
)

const Champions = defineAsyncComponent(
    () => import('../components/Pocket/PocketChampions.vue')
)

const Items = defineAsyncComponent(
    () => import('../components/Pocket/PocketItems.vue')
)

const Runes = defineAsyncComponent(
    () => import('../components/Pocket/PocketRunes.vue')
)
...
<template>
    <PageLayout :key="pocket.key" nav>
        ...
        <template #header-end>
            <div class="absolute -right-2 mb-3 flex h-full py-1">
                <PocketButton :pocket="pocket" :size="26" :padding="7" />
            </div>
        </template>
