...
const { title, class: className, icon, separator, stat, type } = defineProps<{
  stat?: ItemSetStat | StatDetail | StatDetail | undefined
  ...
  >
    <Button
      ...
      :data-type="stat ? `${stat?.winrate}% winrate
        ...
        ${(stat as StatDetail)?.avgTimestamp ? `${(((stat as StatDetail)?.avgTimestamp / 1000) / 60).toFixed(2).replace('.', ':')} acquired` : ''}` : null"
      hover="inset"
