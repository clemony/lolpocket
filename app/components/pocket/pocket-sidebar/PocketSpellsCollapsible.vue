...
<template>
  <Collapsible
    v-model:open="isOpen"
    as="div"
    class="field-box relative">
    <CollapsibleTrigger
      class="w-full group/state group"
      @click.stop.prevent>
      <Btn
        field
        class="w-full h-22 pl-4 pr-2 py-3 [&_.trash-button]:hidden">
        <LazySpellSet
          :set="pocket.spells[0]"
          :pocket="pocket"
          :set-index="0"
    ...
    <LazyCollapsibleContent class="CollapsibleContent ">
      <VueDraggable
        ...
        <div
          ...
          :key="i"
          class="draggable-spells cursor-move items-center w-full first:absolute first:pr-12 first:w-94 first:top-3 first:left-4 h-16 flex gap-7 "
          @click="handleClick(i)">
          <LazySpellSet
            :set="set"
            :pocket="pocket"
