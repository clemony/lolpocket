...
<template>
    <div class="w-2/3">
        <ScrollArea
            class="mb-6 flex h-full max-h-64 w-full flex-row flex-wrap overflow-y-scroll">
            <Label
                variant="ghost"
                size="icon"
                v-for="icon in iconStore"
                class="border-1 m-1 aspect-square max-w-12 self-center border-transparent p-2 has-[:checked]:bg-base-200/60 has-[:checked]:shadow-standard">
                <input
                    v-if="pocket"
                    type="radio"
                    name="iconPicker"
                ...
                <icon :icon="icon" class="shrink-0 text-base-content" />
            </Label>
            <label
