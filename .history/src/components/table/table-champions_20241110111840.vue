...
<ContextMenu class="context-menu">
    <ContextMenuTrigger
        class="group  w-[200px] size-full cursor-pointer items-center justify-start content-center relative flex px-2 gap-3"
        v-if="pocket">
        <template v-for="(champion, index) in pocket.champions[0].champions">
            <div v-if="index < 4" class='overflow-hidden size-14 rounded-lg shadow-sm border border-base-300'>
                <loadImg :url="`/img/champions/${clean(champion.name)}.webp`"
                    class='size-14 object-cover scale-[108%]' />
            </div>
        </template>
