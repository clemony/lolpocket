...
<template>
    <ScrollArea
        class="mb-6 flex h-full max-h-64 w-3/4 flex-wrap justify-self-end overflow-y-scroll">
        <label
            v-for="icon in iconStore"
            class="border-1 btn btn-ghost btn-sm m-1 aspect-square size-12 self-center border-transparent p-2 has-[:checked]:bg-base-200/60 has-[:checked]:shadow-standard">
            <input
                v-if="pocket"
                type="radio"
                name="iconPicker"
