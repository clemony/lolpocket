...
<template>
    <div
        class="grid h-[333px] max-h-[333px] w-full auto-rows-min grid-cols-[1fr_3fr] overflow-hidden duration-500 animate-in animate-out slide-in-from-right-full slide-out-to-right-full">
        <div>
            <label class="">
                ...
                <input
                    ...
                    v-model="isShown"
                    class="hidden"
                    @change="emit('update:isShown', isShown)" />
            </label>
        </div>

        <ScrollArea
            class="max-h-inherit col-start-2 flex h-full !justify-evenly justify-self-center overflow-y-auto rounded-md">
            <label
