 ...
 */
export function useSmartShortcut(options: SmartShortcutOptions): {
  keysPressed: Ref<Set<string>>;
} {
  const { keys, contextSelector, onTrigger, preventDefault = true } = options;
  ...
  const handleKeyDown = (e: KeyboardEvent): void => {
    ...
    if (e.metaKey) activeKeys.add("Meta");
    ...
    const isMatch =
      ...
      normalizedKeys.every(
        (k) => activeKeys.has(k) || activeKeys.has(k.toLowerCase()),
      );
  ...
  const handleKeyUp = (e: KeyboardEvent): void => {
    ...
    if (!e.ctrlKey) keysPressed.value.delete("Control");
    if (!e.metaKey) keysPressed.value.delete("Meta");
    if (!e.altKey) keysPressed.value.delete("Alt");
    if (!e.shiftKey) keysPressed.value.delete("Shift");
  };
