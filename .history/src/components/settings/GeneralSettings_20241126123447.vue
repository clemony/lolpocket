...
<template>
    <main class="w-full">
        ...
        <div class="grid w-full grid-cols-3 gap-6 [&_>*]:size-full">
            <DisplayCard class="grid h-full grid-rows-2">
                <template #header>
                    <h4>Reduce Motion</h4>
                </template>
                <template #description>
                    <div class="flex h-full grow">
                        Get rid of the wiggly wobblies.
                    </div>
                </template>
                <label class="label mx-1 flex cursor-pointer items-end gap-4">
                    <input
                        type="checkbox"
            ...
            <DisplayCard class="grid">
                ...
                <label
                    class="end label mx-1 flex cursor-pointer items-end gap-4 self-end">
                    <input
                        type="checkbox"
                        v-model="gs.reducedMotion"
                        :checked="gs.reducedMotion == false"
