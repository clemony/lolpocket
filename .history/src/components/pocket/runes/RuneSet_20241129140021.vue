...
<template v-if="set">
    <template v-for="rune in whichOne">
        <div
            class="grid size-11 place-items-center rounded-full opacity-70 shadow-inner ring-base-300 brightness-100"
            :class="{
            ...
            <LoadImg
                v-if="rune.name != 'none'"
                :url="rune.img"
                class="size-11 overflow-hidden rounded-full !bg-transparent" />
            <img
                v-else
                src="/img/runes/blankRune.webp"
                class="size-11 overflow-hidden rounded-full" />
        </div>
    </template>
</template>
<style scoped></style>
