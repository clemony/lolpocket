...
<template>
  <div :key="set.name" class="field-box group/set px-2 pt-4 pb-3">
    ...
    <Popover v-model:open="open" @update:open="onOpen()">
      <PopoverTrigger
        class="field-legend z-0 h-10 max-w-90 min-w-54 cursor-text transition-colors duration-300 *:font-semibold *:tracking-tight open:opacity-0 hover:border-bc/60"
      >
        {{ set.name || "Set name..." }}
      </PopoverTrigger>
      <PopoverContent
    ...
    <Popover>
      ...
      >
        ...
        >
          <icon name="more" class="absolute size-4 shrink-0 text-nc/70!" />
        </Button>
      </PopoverTrigger>

      <LazyPopoverContent align="end" :side-offset="1" class="w-64">
    ...
    >
      ...
      >
        <div class="size-full">
          <Button
            variant="neutral"
            shape="square"
            class="group/x absolute -top-2 -right-2.5 z-2 grid size-6.5 size-6.5! scale-0 hover-ring place-items-center overflow-hidden rounded-full opacity-0 transition-all duration-200 ease-spring-bouncy group-open/x:scale-100 group-open/x:opacity-100 group-hover/x:scale-100 group-hover/x:opacity-100"
            @click="removeItemFromSet(pocket, itemSet, item)"
          >
            <icon name="x" class="absolute size-3.5 shrink-0 text-nc/70!" />
          </Button>
        </div>
      </LazyItemPopover>
      ...
      <transition-slide>
        <div
          ...
          class="animate-in zoom-in no-drag size-20 rounded-lg border-0 bg-b3/40 shadow-sm shadow-black/7"
        />
      </transition-slide>
    </div>
  </div>
