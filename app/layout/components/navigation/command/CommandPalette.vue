...
import { asRouteItem } from "./build/helpers"
...
<template>
  ...
  <UCommandPalette
    ...
    <template #item-trailing="{ item }">
      <Icon
        v-if="asRouteItem(item)?.trailingIcon"
        :name="asRouteItem(item).trailingIcon ?? 'i-link'"
        :class="
          cn(
            'opacity-50 group-hover/item:opacity-100',
            asRouteItem(item)?.ui?.itemTrailingIcon
          )
        " />
    </template>

    <template #champion-command="{ item }">
      <ChampionCommand :id="Number(asRouteItem(item).id)" />
    </template>

    <template #item-command="{ item }">
      <ItemCommand :id="Number(asRouteItem(item).id)" />
    </template>

    <template #rune-command="{ item }">
      <RuneCommand :id="Number(asRouteItem(item).id)" />
    </template>

    <template #spell-command="{ item }">
      <SpellCommand :id="Number(asRouteItem(item).id)" />
    </template>
  </UCommandPalette>
</template>
