...
<template>
    <div
        v-if="pocket"
        class="bg-base-200/20 h-full w-full overflow-y-scroll overscroll-none pt-8">
        <!-------------------------------- HEADER --------------------------------->

        <div
            class="frost border-b-base-300 absolute top-0 left-0 z-40 flex h-fit! w-full flex-wrap items-center justify-end gap-4 border-b px-2 py-1">
        ...
        <div class="z-0 flex flex-col overflow-y-scroll">
            <VueDraggable
                ...
                <label
                    ...
                    :key="set.key"
                    :class="set.primary"
                    class="gradient overlay after:from-base-200/30 after:via-base-200 after:to-base-200 pointer-events-auto relative z-0 grid h-16 cursor-pointer items-center overflow-hidden transition-all duration-1000 after:absolute after:right-0 after:z-10 after:h-full after:w-[200%] after:bg-linear-to-r after:via-55% odd:after:brightness-[108%] even:after:brightness-[106%] hover:after:-right-1/2 has-[#runeSets:checked]:after:-right-full">
                    <div
                        class="group border-base-200 z-20 flex h-full w-full items-center gap-4 border-b bg-clip-padding bg-right p-2 transition-all duration-200">
                        <input
                            type="radio"
                            :value="index"
                            id="runeSets"
                        ...
                        <div
                            ...
                            <img
                                ...
                                class="drop-shadow-soft h-11" />
                            <img
                                v-else
                                src="/img/runes/blankRune.webp"
                                class="ring-base-300/90 h-8 rounded-full opacity-70 ring-1 group-hover:opacity-90"
                                id="blank" />
                            <img
                                v-if="
                                    set.secondary != 'none' &&
                        ...
                        <button
                            class="rounded-btn hover:bg-base-200 flex items-center justify-center justify-self-end py-1 opacity-50 hover:opacity-100"
                            alt="menu">
                            <icon
