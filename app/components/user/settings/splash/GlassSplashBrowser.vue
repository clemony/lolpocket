...
const btnClass =
  "  border-white/30 *:opacity-50  hover:bg-p0/30 hover:opacity-100 "
</script>

<template>
  <UModal v-bind="forwarded">
    <UButton
      :class="
        cn(
          'group/icon hover:ring-b4 pointer-events-auto relative z-0 grid aspect-square size-fit shrink-0 cursor-pointer place-items-center self-center rounded-full shadow-xs ring ring-transparent ring-offset-2 ring-offset-transparent drop-shadow-sm transition-colors duration-300 hover:ring-offset-neutral',
          props.class
        )
      ">
      <slot />
