...
<template>
    ...
    <VDropdown theme="select"
        ...
        <button @click.stop.prevent class='flex items-center justify-start w-full h-full '>


            <div class='w-16 -ml-1 overflow-hidden truncate justify-self-start'> {{ typeText }}</div>
            <icon icon="teenyicons:down-small-outline" class="size-3.5 absolute right-1" />
        ...
        <template #popper>
            ...
            <div
                ...
                <label v-for="c in ds.typeOptions" v-close-popper>
                    {{ c }}
                    <input type="radio" v-model="type" :value="c" class="hidden peer" />
                </label>
                <label v-close-popper>
                    none
                    <input type="radio" v-model="type" value="" class="hidden peer" @change="update" />
                </label>
            </div>
        </template>
    </VDropdown>
