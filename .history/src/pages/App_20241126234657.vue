...
<template>
    ...
    <main
        ...
        <div
            v-show="state == 'open'"
            class="command z-[100] mt-24 delay-300 animate-in fade-in-0 slide-in-from-bottom-8">
            <Command />
        </div>
        <label>
            <input
            ...
            <SidebarProvider
                class="page !ease-[cubic-bezier(.97,.24,.38,.98)] overflow-hidden rounded-xl bg-base-100/95 animate-in zoom-in-[0.75] slide-in-from-bottom-1/3"
                :class="{
...
.page,
.command {
    @apply transition-all duration-500 [animation-fill-mode:forwards];
}
</style>
