        ...
        <template #header>
            ...
            <div class='w-fit'>
                <InputEditable v-model:modelValue="set.name" :defaultValue="set.name" @click.stop
                    class='!text-base font-medium text-start items-center align-baseline'>

                    <Badge size="sm"
                        class="shadow-sm gradient border-none backdrop-opacity-40 aspect-square  ![background-size:100%] "
                        @click="
                            set.name = generateRandomName() + ' Set'">
                        <icon icon="fa-solid:question" class='' />
                    </Badge>
                </InputEditable>
            </div>
