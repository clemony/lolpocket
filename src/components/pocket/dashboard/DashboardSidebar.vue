    ...
    <div v-if="pocket" :key="pocket.key" class="flex flex-col">
        <Card class="shadow-smooth">
            <CardContent class="h-36 overflow-hidden rounded-lg p-0">
                ...
                <div
                    ...
                    <p
                        class="items-center whitespace-pre-line text-pretty px-12 align-middle font-serif font-size-6 italic tracking-wide [text-align-last:right]">
                        {{ formattedQuote() }}
                    </p>
                </div>
            </CardContent>
        ...
        <Tabs default-value="spells" class="w-full">
            ...
            <TabsContent value="notes">
                <div class="h-inherit">
                    <textarea
                        class="peer flex size-full cursor-pointer items-start overflow-y-scroll rounded-[4px] border border-b2/40 bg-b2/25 px-1.5 py-1 align-top text-[10px] font-light leading-3 tracking-[0.015em] outline-hidden transition-all duration-300 [resize:none] first-line:indent-2 placeholder:italic hover:border-b3/70 hover:opacity-70 hover:shadow-inner focus:border-b3/70 focus:opacity-100 focus:shadow-inner"
                        v-model="notesInput"
                        @blur="updateNotes"
                        @click.stop
