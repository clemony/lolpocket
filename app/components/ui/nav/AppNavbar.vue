...
<template>
  <nav
    class="w-full justify-between flex fixed flex-nowrap top-0 inset-x-0 w-screen h-(--navbar-height) items-center border-b border-b-b3/60 px-3 z-10">
    <div
    ...
    <NavigationMenu disable-pointer-leave-close>
      <NavigationMenuList class="gap-x-2">
        <NavigationMenuLink
          class="btn !btn-square btn-ghost size-12"
          @click="navigateTo('/')">
        ...
        <template
          ...
          <NavigationMenuItem v-else-if="item.links">
            ...
            <LazyNavigationMenuContent
              ...
              <component
                :is="item.component"
                @open:pins="pinsOpen = true"></component>
            </LazyNavigationMenuContent>
          </NavigationMenuItem>
        </template>
      </NavigationMenuList>
      ...
      <NavigationMenuViewport
        class="!mt-2 w-(--reka-navigation-menu-viewport-width) h-(--reka-navigation-menu-viewport-height)" />
    </NavigationMenu>

    <NavigationMenu
      ref="userNavRef"
      v-model:model-value="userNav"
      ...
      <NavigationMenuList class="justify-end px-1 z-1">
        <NavigationMenuItem
          ...
          <NavigationMenuTrigger
            :arrow="false"
            @click="isDesktop() ? navigateTo('/backpack') : null">
            <hicon
              :name="item.icon.name"
              :class="cn('dst shrink-0', item.icon.class)" />
          </NavigationMenuTrigger>
      ...
      <NavigationMenuViewport
        ...
        :class="
          cn(' translate-x-1   translate-y-2', { '': userNav == 'signIn' })
        "
        :style="{
